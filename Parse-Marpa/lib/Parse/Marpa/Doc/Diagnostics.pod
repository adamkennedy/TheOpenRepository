...
C<Parse::Marpa::Evaluator::value> method,
but something is still wrong,
it can help to run the
C<Parse::Marpa::show_tree> static method after the call to C<next>.
...
C<show_bocage>,
C<show_tree>,
and C<trace_lex> can be lengthy.
...
the corresponding Earley item and QDFA state are reported
