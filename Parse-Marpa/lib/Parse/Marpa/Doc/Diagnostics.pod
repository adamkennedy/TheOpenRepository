...
if the user attempts to compile,
...
shows the LR(0) item corresponding to its and-node,
indicates if there is a Perl closure for the and-node,
and gives the and-node's argument count (or rule length).
...
Or-productions are shown only if verbose output is requested.
