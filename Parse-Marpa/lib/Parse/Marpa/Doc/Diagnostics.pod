...
and describes those Marpa methods and options whose main use is
tracing and debugging.

=head2 Basic Debugging Techniques

First look at the place where parsing was exhausted.
That, along with inspection of the input and the grammar,
is often enough to spot the problem.
Typically you'll have tried that before consulting this
document.
...
the output from each of C<show_earley_sets>,
C<show_bocage>,
C<show_tree>,
and the outputs of C<trace_lex> and C<trace_values>
can be lengthy.
...
Requires knowledge of Marpa internals.
These priorities are set during precomputation.
...
Traces values as they are pushed onto
the evaluation stack, popped off it, calculated and pushed back on.
Very helpful.
...
after the manner of parse forest grammars.
Each production of the parse bocage grammar
corresponds to an and-node,
a possible production of a parse tree.
The lhs is the name of an or-node
and rhs symbols are either or-nodes or the
value of tokens.

Every or-node in the parse bocage will appear one or
more times on the lhs of a production.
Where an or-node appears more than once
on the lhs of a production in a parse bocage grammar,
those productions represent different ways
in which a parse tree may be derived at that or-node.

The optional I<flag> argument, if a true value,
requests verbose output.
Verbose output shows, for each parse bocage
production,
the LR(0) item corresponding to its and-node,
the closure pointer (if there is a closure)
and the and-node's argument count (or rule length).
...
Also in verbose output, when the node tree has a closure,
the value of its pointer is given.
