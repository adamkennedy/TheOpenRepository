...
document.
If you've gotten this far without turning on
Marpa's C<warnings> option (it's on by default),
you probably should turn it on now.

When a problem is not obvious,
it is often useful to turn on the C<trace_lex> option.
...
But if you are interested in doing a complete investigation of a parse, you'll want to
do the following:
...
=item * Run C<show_symbols> on the grammar.

=item * Run C<show_rules> on the grammar.

=back

The L<internals document|Parse::Marpa::Doc::Internals>
has example outputs from the C<show_SDFA> and C<show_status> methods and explains how to read them.

=head1 OPTIONS

These are Marpa options.
Unless otherwise stated,
these Marpa options are valid and useful
...
and at every point in the parse.
...
May be usefully be set at any point in the parse.
...
and use the C<trace_rules> named argument in the same call,
it will take effect B<after> the processing of the C<source> option,
which is probably not what he intended.
The workaround is to
set C<trace_rules> in a method call prior to the one with the C<source> option.
...
=head2 Static Parse::Marpa Method
...
Used for in test scripts.
For debugging and tracing,
the warnings option is usually a more convenient
...
and throwing an exception on the first problem would
...
If the user attempts to compile or to create a parse from
a grammar with problems, an exception is thrown, with the string
returned by C<show_problems> as part of the message.
...
for any reason.
Rules marked "!useful" include not just the ones called useless in
