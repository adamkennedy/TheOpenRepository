...
I<any input>.
The parse bocage grammar describes only those parses which are possible
with I<the specific input> which was accepted by the recognizer.
...
Z<>
    ...
    Note that productions with or-nodes on the lhs only appear in verbose output
