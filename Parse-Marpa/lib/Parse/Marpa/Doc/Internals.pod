...
Marpa does not an Earley item more than once.
Instead of creating a second Earley item with the same QDFA state and
origin at the current earleme,
Marpa adds a source choice to the existing Earley item.
...
a single rule.
A rule which could apply in creating a parse tree is called
an applicable rule.
Every pairing of source choice and applicable rule
...
are evaluated, see L<the section on creating the parse bocage, below|"THE PARSE BOCAGE">.
...
comes from the default end of parsing.
...
No Earley item matches a mortar or-node.

=head2 Matching and Equally Complete Earley Items

Every brick or-node matches an Earley item,
based on its rule, dot position, and earleme location.
An or-node is said to B<match> an Earley item if
...
a nulling symbol,
so that mortar or-nodes don't match any Earley items.
The B<equally complete Earley item> to an or-node is
the Earley item which has a
current location
the same as
the earleme location of the or-node;
and which has the QDFA state
equally complete to the or-node.
A QDFA state is B<equally complete> to an or-node if
the QDFA state contains an LR(0) item which is
equally complete to the or-node.
A Marpa LR(0) item is B<equally complete>
to an or-node
if the LR(0) item has the same rule as the or-node,
and either the LR(0) item's dot position is the same as the or-node's dot position,
or the only symbols between the dot position
of the LR(0) item and the dot position of the or-node are nulling symbols.
If an Earley item matches an or-node,
then that Earley item and that or-node are equally complete.
...
along with the Earley item equally complete to the or-node,
...
of course, also the first child.
...
the source choices of the equally complete Earley item.
...
=head2 Creating the Parse Bocage

Each phase of creation of a parse bocage starts with an or-node.
In this section, we'll call this the parent or-node, or simply the B<parent>.
For the initial step in creating the parse bocage, the parent is the start or-node.
In this section, we'll call the and-node children of the parent, the and-node children,
or where it's clear, simply the B<children>.
In this section, we'll call the or-node children of the and-node children,
the B<grandchildren>.

For every or-node parent,
there is a set of productions which are used to create
the and-node children,
and the cause and predecessor grandchildren.
The productions in this set are the B<applicable rules>.
When creating the and-node children,
Marpa considers every pairing of an applicable rule and a source choice.
...
every completed rule in the QDFA state of
the matching Earley item whose lhs is the
same as the child lhs symbol
is an applicable rule.

If there are any symbols before a parent or-node's pre-dot symbol,
its and-node children will have predecessor grandchildren.
These predecessor grandchildren will be kernel or-nodes.
As its rule, each predecessor grandchild will have one of the applicable rules.
If the parent or-node was a closure or-node,
the predecessor grandchild's dot position will be in front of the last symbol of the rule.
If the parent or-node was a kernel or-node,
the predecessor grandchild's dot position will be one less than the parent or-node's dot position.
That means that the pre-dot symbol of the parent or-node will be the post-dot symbol
of the or-node which is its predecessor grandchild.

=head2 Naming Or-Nodes

The name of an or-node is the name of its
equally complete Earley item,
...
The equally complete Earley item is S3@0-1.
...
much of the work of parsing a specific input
has been done.
