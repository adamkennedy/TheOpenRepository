...
there is a transtion from S1 to S4 on C<number>.
...
SDFA state 6.  Here's S6@0-3, the item which is the effect:
...
The non-nullable version of the original symbol is not allowed to appear in places
...
the non-nulling symbols and their non-nulling variants.
