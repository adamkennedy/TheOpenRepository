...
which is also the start of the rules
in the LR(0) items,
...
must often represent the original grammar's
...
a form that has at most two symbols on the right hand side.
...
Or-nodes with their rule position at the end of the rule are B<closure or-nodes>.
...
If the position is at a non-nulling symbol,
...
Whether or not an or-node is a closure or-node
has nothing to do with whether or not it has a
...
("C<+>").
The kernel or-node contains information for
