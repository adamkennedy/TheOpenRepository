...
If an B<Earley item> of the correct form is in the right Earley set
...
the Earley sets as input.

Each Earley set corresponds to an earleme location.
Earley set 0 is at the beginning of input, before any tokens are recognized.
Earley set 1 is after the first earleme (earleme 0), but before the second (earleme 1).
Earley set I<N> is after earleme I<N-1>, but before earleme I<N>.

An Earley set is a set of Earley items.
Each Earley item has a B<QDFA state>, an B<origin> and a B<current earleme>.
...
Every LR(0) item is a statement about the recognition of a rule,
or of part of a rule.
Presence of the LR(0) item
in the QDFA state of an Earley item
indicates that the rule has been
recognized as far as the dot position.
If the dot is at the beginning of the rule in an LR(0) item,
the part of a rule recognized is zero length,
and the LR(0) item is a prediction.
...
an or-node is matched with an Earley item.
An or-node with its dot position
at the end of the rule,
or just before a non-nullable symbol,
is called a B<brick or-node>.
Every brick or-node is in an exact match with an Earley item.
The Earley item which matches a brick or-node
has a dot location which exactly matches the earleme location
of the brick or-node and
has a QDFA state which contains an LR(0) item
which exactly matches the brick or-node's rule and dot position.

There is an or-node for every symbol of any possible parse.
This means there are or-nodes which have nulling post-dot symbols.
Because they fill the cracks in between brick or-nodes,
these or-nodes are called B<mortar or-nodes>.

No Earley items correspond exactly to mortar or-nodes.
This is because Marpa has no LR(0) items with dot positions before nulling
symbols.

The Earley item used in the name of a mortar or-node is the Earley item
which exactly matches the rule and the earleme location of the mortar or-node,
and which contains the "next" LR(0) item for the or-node's rule.
The "next" LR(0) item is the one with its rule position in front of the first non-nullable
symbol after the mortar or-node's dot position.
If every symbol in the rule after the or-node's dot position is a nulling symbol,
the "next" LR(0) item is the one with its dot position at the end of the rule.

Or-nodes with their dot position at the end of the rule are B<closure or-nodes>.
All other or-nodes are B<kernel or-nodes>.

Or-nodes names consist of the name of the corresponding Earley item,
followed by a label indicating their B<selection criteria>.
The selection criteria depend on whether the or-node is a kernel
or a closure or-node.

The selection criterion for a closure or-node is a symbol,
called its B<selection symbol>.
Note that, in addition to a selection symbol,
a closure or-node has a pre-dot symbol,
sometimes called simply the or-node's symbol,
and this will usually not be the same as its B<selection symbol>.

The selection symbol
is used to restrict the rules that will be used
in the child nodes of the closure or-node.
As mentioned, in a parse bocage, all children of or-nodes 
are and-nodes,
and every and-node represents a rule,
or a piece of a rule,
of the original grammar.
An and-node may only be the child of a closure or-node,
if the rule it represents
has the selection symbol as its lhs.
In the Earley item corresponding to the closure or-node,
several completed rules may have the same lhs,
and all these rules will meet
the or-node's selection criteria.

The selection criteria for a kernel or-node is
a rule, and a rule position.
If the position is at a non-nullable symbol,
exactly one LR(0) item will match the selection criteria,
and the child and-nodes will be created using that LR(0) item.
If the position is at a nulling symbol,
a child and-node will be created using
the null value of the symbol.

=head2 Naming Or-Nodes

The name of a kernel or-node is the name of its
...
and its rule position,
colon-separated.
The "R" in the label is a mnemonic for "rule".

For example,
the kernel or-node named S3@0-1R0:1 corresponds
to Earley item S3@0-1.
The selection criteria are rule 0 at position 1
(C<< e -> e . op e >>).

For closure or-nodes, the selection criteria label is the
letter "L", followed by a symbol number.
...
and it was the effect of completing the symbol
