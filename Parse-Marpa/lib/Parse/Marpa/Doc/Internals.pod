...
are evaluated,
including how Marpa determines which rules are applicabile,
...
=head2 The Relationship between Or-Nodes to Earley Items
...
and the only symbols between the dot positions
of the or-node and the dot position of the LR(0) item
...
and the parse will end at its current earleme.

In normal use,
the child lhs symbol of the start or-node is set to the start
symbol of the grammar,
and the corresponding Earley item for the start or-node is set to
...
=head3 Creating the And-node Children for the Parent Or-node
...
we'll call the or-node currently being processed,
the parent or-node, or simply the B<parent>.
We'll call the and-node children that are created for this parent, the and-node children,
...
Earley item.
A rule is applicable if it is the rule in a completed LR(0) item
...
in the corresponding Earley item.
...
same as dot position of the parent or-node's child criteria.
If the parent or-node was a closure or-node,
the dot position will be at the end of the and-node's rule.
...
an and-node is created with a token whose value is the
the null value of that symbol.
The and-node is also created with a predecessor or-node.
The corresponding Earley item of the predecessor or-node
will be the Earley item of the parent or-node.
The child criteria for the predecessor or-node will be
the and-node's rule,
...
based on its corresponding source choice.
If it was a token source choice, the and-node will have
the token value from the source choice.
If it was a completion source choice, the and-node will have
a cause or-node.
The corresponding Earley item for the cause or-node will be
Earley item which was the cause in the source choice.
The child lhs symbol for the cause or-node will be the pre-dot
symbol of the and-node.

If the and-node's pre-dot symbol is non-nulling,
a predecessor or-node is created as its child
if and only if its corresponding source choice
had a predecessor item.
If a predecessor or-node child is created for an
and-node,
its corresponding Earley item will be the predecessor
item of the and-node's corresponding source choice.
The child criteria for the predecessor or-node will be
the and-node's rule,
with a dot position before the and-node's pre-dot symbol.
...
and its and-nodes created and linked into the parse bocage,
Marpa checks to see if any more or-nodes remain on the list
to be processed.
As the last section showed,
when Marpa creates and-nodes,
it often creates, as their children, new predecessor and cause or-nodes.
These must be added to the list of or-nodes yet to be processed.
Or-nodes must be pulled from the list and processed until there are no more.
