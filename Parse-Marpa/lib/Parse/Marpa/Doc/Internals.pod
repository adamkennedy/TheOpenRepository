...
comes from the default end of parsing which,
in offline mode,
is usually
...
or a piece of one.
And-nodes and or-nodes strictly alternate in MBF.
...
Each or-node corresponds to a rule,
a dot position in that rule,
and an earleme location.
The symbol before the dot position in a rule
is the or-node's B<pre-dot symbol>.
The symbol after the dot position in a rule
is the or-node's B<post-dot symbol>.

The B<or-node symbol> is its pre-dot symbol.
Or-nodes are not created for dot positions
at the beginning of rules,
so every or-node has an or-node symbol.

An or-node whose dot position is at the end
of its rule is a B<closure or-node>.
Any other or-node is a B<kernel or-node>.
If the post-dot symbol of a kernel or-node is
nulling, it is a B<mortar or-node>.
Any other kernel or-node,
...
Mortar or-nodes exist,
because Marpa has no LR(0) items with nulling post-dot symbols.
Mortar or-nodes are needed because there must be an or-node
for every symbol in every rule used in any possible parse.
No Earley item exactly matches a mortar or-node.

=head2 Matching Or-nodes and Earley Items

Every brick or-node exactly matches an Earley item,
based on its rule, dot position, and earleme location.
An Earley item is said to B<exactly match> an or-node if
the current location of the Earley item
is the same as
the or-node's earleme location;
and for one of the LR(0) items
in the Earley item's QDFA state,
the rule and the dot position
are the same as the rule and dot position
of the or-node.

Marpa LR(0) items cannot have the dot position before
a nulling symbol,
so that mortar or-nodes don't exactly match any Earley items.
An Earley item is the B<nearest match> to an or-node
if everything else about it is a match,
and the dot position of an LR(0) item in the Earley item's
QDFA state is
at or before
the or-node's dot position
and as close as possible.

To say the same thing more precisely,
let's call an LR(0) item B<Aycock-Horspool-correct>
if its dot position is not in front of a nulling symbol.
Call one LR(0) item earlier than, later than, or the same as another LR(0)
item with the same rule,
according to whether the first LR(0) item's dot position is earlier than,
later than, or the same as the second LR(0) item's dot position.
Let the distance between two LR(0) items with the same rule be
the distance, counted in symbols, between their dot positions.
An Earley item is the nearest match to an or-node if
the current location of the Earley item
is the same as
the earleme location of the or-node;
and the QDFA state of Earley item contains the
least distant Aycock-Horspool-correct LR(0) item
that is earlier than or the same as the or-node's LR(0) item.
By these definitions, if an Earley item exactly matches an or-node,
it is also the nearest match.

=head2 Child Criteria and Child Nodes

Every or-node is a parent of one or more and-nodes.
Every and-node is also a parent.
Children of and-nodes can be either tokens or or-nodes.

Every or-node has child criteria, which are used,
along with the or-node symbol,
to create the or-node's children.
The child criterion for a closure or-node is a symbol,
called its B<child lhs symbol>.
The child criteria for a kernel or-node are
a rule, and a dot position.

In turn,
each and-node either has just one child, called a B<cause>,
or two children, a B<predecessor> and a B<cause>.
The predecessor, if present, always comes first.
The cause is always last.
When the cause is the only child it is,
of course, also first.

A cause can be either a closure or-node or a token.
If the parent or-node's symbol is nulling,
there will be only one child and-node,
and the and-node's cause will be a token created from
the null value of the symbol.
If the parent or-node's symbol is non-nullable,
child and-nodes are created from
the source choices of the nearest matching Earley item.
Where the source choice is a token source choice, the and-node's cause will be
a token.
Where the source choice is a completion source choice, the and-node's cause will be
a closure or-node.

For every or-node,
there is a set of productions which are used to create
the or-node's and-node children,
and the cause and predecessor grandchildren.
The productions in this set are called the B<applicable rules>.
In creating and-nodes,
Marpa uses every pairing of an applicable rule with a source choice.

Where the parent or-node is a kernel or-node,
the rule in the child criteria is
the only applicable rule.
For closure or-nodes,
there may be more than one applicable rule.
Where the parent or-node is a closure or-node,
every rule in the LR(0) items of the QDFA state of
the exactly matching Earley item whose lhs is the
same as the child lhs symbol
is an applicable rule.

If there are any symbols before a parent or-node's pre-dot symbol,
its child and-node will have a predecessor child.
This predecessor will be a kernel or-node.
As its rule, the predecessor will have one of the applicable rules.
If the predecessor's grandparent or-node was a closure or-node,
the predecessor or-node's dot position will be in front of the last symbol of the rule.
If the predecessor's grandparent or-node was a kernel or-node,
the predecessor or-node's dot position will be one less than its grandparent's dot position.
That means that the pre-dot symbol of the grandparent or-node will be the post-dot symbol
of the predecessor or-node.

=head2 Naming Or-Nodes

The name of an or-node is the name of its
nearest matching Earley item,
followed by a label describing its LR(0) item selection criteria.
...
The nearest matching Earley item is S3@0-1.
The selection criteria are rule 0 at dot position 1
(C<< e -> e . op e >>).

For closure or-nodes,
the selection criteria label is the
...
In this case the exactly matching Earley item is S6@0-3,
...
It has index 0.
A PBG always starts with the start or-production.
...
and rule position for this and-node.
...
There is no kernel or-node, because the current
symbol for this and-production is the first one of
its rule -- in fact, it is the only symbol in its rule.

S4@0-1L0 is a closure or-node with "C<L0>" as its selection
criteria.
Since the current symbol is final,
this is a brick or-node,
and there will be
a corresponding LR(0) item.
The LR(0) item will be in QDFA state 4,
and will be a completed rule with
symbol number 0 on its lhs.
Symbol number 0 is C<e>.
There is, as it happens, only one
...
appendix.
Every node in a Marpa parse tree corresponds
to an and-node
in the parse bocage.
...
every parse bocage and-node corresponds to a tree node.
If the parse bocage contains an ambiguous parse,
some and-nodes will not have corresponding tree nodes.

=begin Parse::Marpa::test_document:

## next display
in_equation_show_tree($_);
...
As Marpa evaluates tree nodes,
it computes values and pushes them onto an evaluation
stack.
When a tree node needs values for its computation,
perhaps because a Perl closure requires them as arguments,
they are popped from the evaluation stack.
The result is pushed back onto the stack.
When Marpa computes the value for the root tree-node,
the result will be the only value left on the stack,
and will be the value of the parse.
...
In other words,
a B<proper nullable> is a symbol that
...
and creating a second symbol to serve as the original symbol's nulling variant.
...
Getting rid of proper nullables on the right hand side
is more difficult than getting rid of them
...
The tags beginning with an "C<x>", like "C<[x5d]>", are arbitrary hex values,
