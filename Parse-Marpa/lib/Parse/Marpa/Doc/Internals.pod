...
but Marpa's method is basically the one they give in
...
is not hard to spot.
Marpa always adds its own, internal, start rule,
...
Parse forests only store parse trees, and parse trees cannot contain cycles.
A parse bocage can store parse graphs as well as parse trees.
Parse graphs can contain cycles.
The ability to store parse graphs means that parse bocages can
represent parses which are infinitely
ambiguous.

Parse bocages, like parse forests, consist of and-nodes
and or-nodes.
As in parse forests,
the or-nodes of a parse bocage
represent choices between
...
Marpa's transformation of the grammar into productions
whose rhs is at most length two,
is very similar to Chomsky Normal
Form (CNF).
I will call the form the grammar takes in Marpa's
parse bocage, B<Marpa Bocage Form> (B<MBF>).

Marpa evaluates the grammar in Marpa Bocage Form for
four reasons.
First, MBF is essentially the form in which the grammar
is found in the Earley items.
Every link choice of an Earley item contains at most two links.
...
If Marpa were to rewrite the parse data found in the Earley
sets back into the productions of the original grammar,
it would be spending effort to create a slower and
...
As a result, many or-nodes are trivial -- they
...
If an Earley item was created by following
a predecessor link,
the selection criteria are the rule currently being
derived,
and the current position in that rule.

For example, the or-node named S3@0-1R0:1 corresponds
to Earley item S3@0-1.  That Earley item was reached
at position 1 in a derivation of rule 0 (C<< e -> e . op e >>)

The current position of an or-node will correspond exactly
to the current position of the Earley item,
and to the dot position of one or more of that Earley item's
LR(0) items,
unless the current position is at a nulled symbol.
No Earley items represent nulled symbols,
and Marpa's LR(0) items never have a nulled symbol at the dot position
unless they are completed rules.

But
in the parse bocage
every symbol is represented by a node,
including those nulled symbols which are not the last symbol in a rule.
...
so "nulled or-nodes" use the name of the successor Earley item
unless and until Marpa reaches a
non-nulled symbol.
For these "nulled or-nodes",
the current position of the or-node will be
before the current position of the or-node's corresponding Earley item
and before the dot position of the corresponding
LR(0) items in that Earley item.

=head2 Naming Other Or-Nodes

Following predecessor links is one of the three ways of
creating or-nodes.
Other or-nodes are created by following cause links.
Also, one or-node is created from the start item to initialize
the parse bocage.
If a node was not created by following a predecessor link,
it is called a B<non-predecessor or-node>.

For non-predecessor or-nodes,
the selection criterion is a single symbol.
An example of a non-predecessor or-node is S6@0-3L0.
In this case the Earley item was S6@0-3,
and the predecessor symbol was symbol 0 (C<e>).
...
The name of an and-node is made up of the parent or-node's name,
...
Or-nodes appear in the PBG as one or more productions,
each of
which has the name of an or-node at its lhs,
and each of which had the name of one of
the or-node's child and-nodes
as the only symbol on its rhs.
In the example above, the rhs and-node is the first child
of the lhs or-node.

In many cases, the or-nodes are trivial -- they have
exactly one child.
Trivial or-nodes have only one production in the PBG.
In an unambiguous parse, like the parse of our example,
I<all> the or-nodes are trivial.

Or-nodes are not show unless C<show_bocage> is called
with the verbose option.
The or-node productions are necessary for the PBG to be a real
BNF grammar,
but they convey no information not easily deducted from
the the and-nodes.

For example, the and-node on the rhs of this production
is S2@0-3L3[0].
That's enough to deduce the contents of this or-node production.
The name of the parent or-node of an and-node will always be 
the name of the and-node, minus its bracketed index.
In this case, the parent of 
S2@0-3L3[0] will be
S2@0-3L3.
Whenever, there is an and-node in the PBG,
there is always an or-node production with the
unbracketed name of the and-node node on it lhs
and the and-node itself as the
only rhs symbol.
...
Above is the production for the and-node which was the only
child of the or-node whose production was the previous example.
Parse bocage and-nodes always have one or two rhs symbols.
A token or a cause or-node may be on the right hand side.
One or the other is always present, but never both.
Additionally, there may be a 
predecessor or-node.
If a predecessor or-node is present,
it is always the first symbol.

In this example, the only symbol on the rhs is
a cause or-node: S6@0-3L0.
The next line contains a dotted rule showing the rule
and position within the rule for this and-node.
The last line is not very informative -- it tells
us the length of the rule's rhs, and that there is
a closure corresponding to this rule.

    Corresponds to the rule?  Check.
