...
The start of the rules in the LR(0) items
is the origin of the Earley item.
...
The input is recognized successfully if, at the last earleme, there is
...
Every brick or-node is in an exact match with one Earley item.
...
All LR(0) items used to create and-node children of a closure or-node
must be completed rules
where the symbol in the selection criterion is the lhs of the rule.
In an Earley item,
...
The dot symbol ("C<op>") matched the token
("C<+>").
The kernel or-node (S3@0-1R0:1)
contains information for
deriving the symbols in rule 0 before the dot symbol.
In this case there is only one symbol
("C<e>")
before the dot symbol.
