...
All or-nodes,
except those with a nulling post-dot symbol,
exactly match an Earley item.
Saying that an or-node
exactly matches an Earley item
means that
the or-node's earleme location exactly matches the current location
of the Earley item;
and the or-node's rule and dot position
exactly match the rule and the dot position
in one of the LR(0) items
in the Earley item's QDFA state.

If an or-node matches an Earley item exactly,
it is called B<brick or-node>.
Or-nodes which do not match an LR(0) item in any
Earley node are called B<mortar or-nodes>,
the idea being that
...
Mortar or-nodes are necessary so that there can be an or-node
for every symbol in every rule used in any possible parse.
Mortar or-nodes are not an exact match to any Earley item.

Mortar or-nodes, like brick or-nodes, are named after Earley items.
...
The "next" LR(0) item is the one whose post-dot symbol is the first non-nullable
symbol after the mortar or-node's dot position.
If there is no non-nullable symbol after the mortar or-node's dot position,
...
Remember that, in addition to a selection symbol,
a closure or-node has a pre-dot symbol,
sometimes called simply the or-node's symbol,
and this will usually not be the same as the or-nodes's B<selection symbol>.
...
For an and-node to be the child of a closure or-node,
the rule it represents must have the selection symbol as its lhs.
...
If the pre-dot symbol is non-nullable,
the child and-nodes will be created from the source choices.
