...
SDFA states are not traditional --
they were invented by Aycock and Horspool.
In the traditional terminology,
the Earley set corresponding to the
the origin earleme is rather confusingly called the "parent".
And in the traditional notation for an Earley item,
the dot earleme is not
included at all -- the context is supposed to be sufficient to tell you
which Earley set each Earley item is in.)
...
This states that this item is for SDFA state 1,
...
may be ignored.
They are the numbers of the NFA states that were combined into this SDFA state.
Marpa uses SDFA states to track the parse.
Every SDFA state has one or more B<LR(0) items>.
...
A dot at the beginning of an LR(0) item indicates that the rule is only predicted
...
C<pointer> is the name of the symbol B<before> the dot pointer in the current choice of
rule.
...
There are a total of 6 proper nullables:
the original 4 plus the 2 symbols newly created to serve as left hand sides.
This is why, in order to have only 2 proper nullables per piece, the original rule
had to be divided into 3 pieces.
The newly created lhs symbols, if they are proper nullables,
