...
But understanding how Marpa works can make you a better grammar writer
...
The reader should also have read the other Marpa documentation.
...
does not actually parse its input.
...
In Marpa, the Earley sets are built as tokens are recognized.

Each Marpa B<earleme> corresponds one-to-one to an Earley set.
An B<Earley> set is a set of Earley items.
Each Earley item has an B<SDFA state>, an B<origin earleme> and an B<dot earleme>.
An Earley item's dot earleme is also called its B<current earleme>.
Here's a representation of an B<Earley item>, as you might see it in
debugging output from Marpa:

    C<S5@6-7>

This Earley item is for SDFA state 5 (that is the "C<S5>" part).
The Earley item originates at earleme 6 and is currently at earleme 7 (that's the "C<@6-7>" part).
The number of an Earley item's dot earleme is always
...
the origin earleme is confusingly called the "parent",
and the dot earleme corresponds to the earley set of the item.)
...
This states that SDFA state 1 originates at earleme 2 and is currently at earleme 2.
...
A dot at the beginning indicates that the rule is only predicted -- none
of its symbols have yet been recognized.
A dot at the end indicates that the rule is complete -- all
of its symbols have been recognized.
A dot between two symbols indicates that all symbols before the dot
have been recognized in the input,
but none of the symbols after the dot have been recognzied.
...
The presence of an SDFA state in an Earley item means that the parses in its LR(0) items
are possible at its current earleme.
The dot position of every LR(0) item in the SDFA state
corresponds to the current earleme of the Earley item.
...
Scanning adds Earley items as tokens are recognized.
...
If the lexer finds a C<number> token with a length (in earlemes) of 1 at earleme 2,
...
But parsing in the strict sense is determining the structure of the input,
...
Predicted items are always the result of empty transitions.
...
At most two of the SDFA states can be completed start rule states.
One is a special case.
In a null parse, the start state, SDFA state 0,
...
The completed start rule state in our example is SDFA state 2:
...
I used their ideas and added a few of my own.
...
the non-nulling and nulling variants of the proper nullables.
