...
The constructor C<Parse::Marpa::Recognizer::new> is called
...
with the C<Parse::Marpa::Recognizer::text> method,
or directly as tokens
with the C<Parse::Marpa::Recognizer::earleme> method.

=item * Initial Parse Evaluation

Once the input is recognized, it can be evaluated.
The first value is computed with the C<Parse::Marpa::Recognizer::initial> method.
The return value from C<initial> indicates success or failure.
The actual value of the parse is returned by the C<Parse::Marpa::Recognizer::value> method.
C<initial> converts the parse object into an iterator,
in case other parses will be evaluated.

=item * Parse Iteration

Parse iteration is continued with the C<Parse::Marpa::Recognizer::next> method,
...
C<Parse::Marpa::Recognizer::value()> method.
