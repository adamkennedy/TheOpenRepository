...
Parse::Marpa::Doc::Concepts - Concepts helpful for Using Marpa
...
L<elsewhere|Parse::Marpa::Doc::Algorithm>.
...
Marpa's C<Parse::Marpa::Recce::text()> method uses a model where
there's B<one earleme per character>.

C<Parse::Marpa::Recce::text()> is the routine used most commonly to provide input
...
scheme.
Input tokens may be fed directly to Marpa with the C<Parse::Marpa::Recce::earleme()> method
...
The constructor C<Parse::Marpa::Recce::new()> is called
...
with the C<Parse::Marpa::Recce::text()> method,
or directly as tokens
with the C<Parse::Marpa::Recce::earleme()> method.

=item * Initial Parse Evaluation

Once the input is recognized, it can be evaluated.
The first value is computed with the C<Parse::Marpa::Recce::initial()> method.
C<initial()'s> return value indicates success or failure.
The actual value of the parse is returned by the C<Parse::Marpa::Recce::value()> method.
C<initial()> converts the parse object into an iterator,
in case other parses will be evaluated.

=item * Parse Iteration

Parse iteration is continued with the C<Parse::Marpa::Recce::next()> method,
...
C<Parse::Marpa::Recce::value()> method.
