...
It is the low-level interface used by all the porcelain interfaces.
...
Marpa does not allow use of both the plumbing and a porcelain interface
...
designed to allow flexibility for the porcelain interfaces.
...
Rule descriptions can be either arrays (the short form)
or hashes (the long form).
...
The values of the C<min> and C<max> rule options must be non-negative integers.
...
C<min> must be greater than or equal to zero.
C<max> must be greater than zero.
In addition, C<max> must be greater than or equal to C<min>.
...
See L<the MDL document|Parse::Marpa::Language> for details on writing terminal regexes.
