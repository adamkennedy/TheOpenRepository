...
C<max> must be undefined or 1.
If C<max> is undefined, C<min> may be undefined.
0 or 1,
If C<max> is 1, C<min> may be 0 or 1.
For any other combination of values,
...
=item * C<min> 0 or 1, and C<max> undefined

The rule is a B<sequence production>.
Only one symbol is allowed on the right hand side.
The right hand side symbol may not be a nullable symbol.
The rhs symbol must be repeated at least C<min> times
and may be repeated an unlimited number of times.

=item * C<min> and C<max> both 1

The rule is an ordinary B<BNF production>.

=item * C<min> 0 and C<max> 1

Only one symbol is allowed on the right hand side,
and it is optional --
that is, it may be present once or not at all.
