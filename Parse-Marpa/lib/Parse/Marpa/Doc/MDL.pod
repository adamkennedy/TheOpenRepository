...
(Cycles are not allowed in Marpa, so this would cause Marpa to
...
a "prefix", another regex which is to be checked for
...
are evaluated in a special namespace dedicated to running actions.
Every call to the C<mdl> static method has its own separate namespace for actions.
...
The code is run in a special namespace special to each recognizer,
but common to all the user code run within the recognizer.
...
Lexing closures finds the string to be matched as a C<pos> offset within
