...
and the current Earley set is I<I>,
a newly scanned Earley item is added to Earley set I<I+L>.
...
In the Aycock-Horspool version of Earley items,
each Earley item has available two links,
one to a child item (its cause), and another to sibling item
(its predecessor).
Either of these may be unused.
In effect, the grammar is rewritten in the Earley items
