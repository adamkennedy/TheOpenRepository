package Perl::APIReference::V5_011_001;
use strict;
use warnings;
...
Usually used via its frontend C<sv_setpvf_mg>.
	...
	void	sv_vsetpvf_mg(SV *const sv, const char *const pat, va_list *const args)','name' => 'sv_vsetpvf_mg'},'sv_destroyable' => {'text' => 'Dummy routine which reports that object can be destroyed when there is no
...
use the Encode extension for that.
	...
	STRLEN	sv_utf8_upgrade_flags(SV *const sv, const I32 flags)','name' => 'sv_utf8_upgrade_flags'},'ibcmp_utf8' => {'text' => 'Return true if the strings s1 and s2 differ case-insensitively, false
...
Does not use C<TARG>.  See also C<XPUSHs>, C<PUSHmortal> and C<PUSHs>.
	...
	void	XPUSHmortal()','name' => 'XPUSHmortal'},'SvVOK' => {'text' => 'Returns a boolean indicating whether the SV contains a v-string.
...
sv_true() or its macro equivalent.
	...
	bool	sv_2bool(SV *const sv)','name' => 'sv_2bool'},'G_VOID' => {'text' => 'Used to indicate void context.  See C<GIMME_V> and L<perlcall>.','name' => 'G_VOID'},'dORIGMARK' => {'text' => 'Saves the original stack mark for the XSUB.  See C<ORIGMARK>.
...
instead.
	...
	void	sv_clear(SV *const sv)','name' => 'sv_clear'},'SvOOK' => {'text' => 'Returns a U32 indicating whether the pointer to the string buffer is offset.
...
(Uses C<sv_setsv>).
	...
	void	sv_setpv_mg(SV *const sv, const char *const ptr)','name' => 'sv_setpv_mg'},'sv_2mortal' => {'text' => 'Marks an existing SV as mortal.  The SV will be destroyed "soon", either
...
C<SvSetMagicSV_nosteal>.
	...
	void	sv_setsv(SV *dstr, SV *sstr)','name' => 'sv_setsv'},'POPs' => {'text' => 'Pops an SV off the stack.
...
argument more than once.
	...
	void	SvGETMAGIC(SV* sv)','name' => 'SvGETMAGIC'},'sv_inc' => {'text' => 'Auto-increment of the value in the SV, doing string to numeric conversion
...
(deprecated - use C<(CV *)NULL> instead)','name' => 'Nullcv'},'get_av' => {'text' => 'Returns the AV of the specified Perl array.  C<flags> are passed to
C<gv_fetchpv>. If C<GV_ADD> is set and the
Perl variable does not exist then it will be created.  If C<flags> is zero
and the variable does not exist then NULL is returned.
...
information on how to use this function on tied hashes.
	...
	SV**	hv_fetch(HV *hv, const char *key, I32 klen, I32 lval)','name' => 'hv_fetch'},'sv_unref_flags' => {'text' => 'Unsets the RV status of the SV, and decrements the reference count of
whatever was being referenced by the RV.  This can almost be thought of
...
number may use \'_\' characters to separate digits.
	...
	UV	grok_oct(const char* start, STRLEN* len_p, I32* flags, NV *result)','name' => 'grok_oct'},'sv_rvweaken' => {'text' => 'Weaken a reference: set the C<SvWEAKREF> flag on this RV; give the
