package Perl::APIReference::V5_011_002;
use strict;
use warnings;
...
information on how to use this function on tied hashes.
	...
	HE*	hv_store_ent(HV *hv, SV *key, SV *val, U32 hash)','name' => 'hv_store_ent'},'gv_stashsv' => {'text' => 'Returns a pointer to the stash for a specified package.  See C<gv_stashpvn>.
...
Usually used via its frontend C<sv_setpvf_mg>.
	...
	void	sv_vsetpvf_mg(SV *const sv, const char *const pat, va_list *const args)','name' => 'sv_vsetpvf_mg'},'pad_findmy' => {'text' => 'Given a lexical name, try to find its offset, first in the current pad,
...
could potentially warn under some level of strict-ness.
	...
	void	sortsv_flags(SV** array, size_t num_elts, SVCOMPARE_t cmp, U32 flags)','name' => 'sortsv_flags'},'XS' => {'text' => 'Macro to declare an XSUB and its C parameter list.  This is handled by
C<xsubpp>.','name' => 'XS'},'scan_oct' => {'text' => 'For backwards compatibility. Use C<grok_oct> instead.

	NV	scan_oct(const char* start, STRLEN len, STRLEN* retlen)','name' => 'scan_oct'},'sv_setpvn' => {'text' => 'Copies a string into an SV.  The C<len> parameter indicates the number of
...
reference count is 1.
	...
	SV*	newSVrv(SV *const rv, const char *const classname)','name' => 'newSVrv'},'strnEQ' => {'text' => 'Test two strings to see if they are equal.  The C<len> parameter indicates
...
Most code should use utf8_to_uvchr() rather than call this directly.
	...
	UV	utf8n_to_uvuni(const U8 *s, STRLEN curlen, STRLEN *retlen, U32 flags)','name' => 'utf8n_to_uvuni'},'av_create_and_unshift_one' => {'text' => 'Unshifts an SV onto the beginning of the array, creating the array if
necessary.
...
to add more than one instance of the same \'how\'.
	...
	void	sv_magic(SV *const sv, SV *const obj, const int how, const char *const name, const I32 namlen)','name' => 'sv_magic'},'dMARK' => {'text' => 'Declare a stack marker variable, C<mark>, for the XSUB.  See C<MARK> and
C<dORIGMARK>.
...
use the Encode extension for that.
	...
	STRLEN	sv_utf8_upgrade_flags(SV *const sv, const I32 flags)','name' => 'sv_utf8_upgrade_flags'},'lex_discard_to' => {'text' => 'Discards the first part of the L</PL_parser-E<gt>linestr> buffer,
...
removed without notice.
	...
	void	lex_discard_to(char *ptr)','name' => 'lex_discard_to'},'ibcmp_utf8' => {'text' => 'Return true if the strings s1 and s2 differ case-insensitively, false
...
http://www.unicode.org/unicode/reports/tr21/ (Case Mappings).
	...
	I32	ibcmp_utf8(const char *s1, char **pe1, UV l1, bool u1, const char *s2, char **pe2, UV l2, bool u2)','name' => 'ibcmp_utf8'},'SvREFCNT_inc_void' => {'text' => 'Same as SvREFCNT_inc, but can only be used if you don\'t need the
...
you don\'t need to do anything.
	...
	PerlInterpreter*	perl_clone(PerlInterpreter *proto_perl, UV flags)','name' => 'perl_clone'},'sv_setnv' => {'text' => 'Copies a double into the given SV, upgrading first if necessary.
Does not handle \'set\' magic.  See also C<sv_setnv_mg>.

	void	sv_setnv(SV *const sv, const NV num)','name' => 'sv_setnv'},'sv_2nv' => {'text' => 'Return the num value of an SV, doing any necessary string or integer
conversion, magic etc. Normally used via the C<SvNV(sv)> and C<SvNVx(sv)>
macros.

	NV	sv_2nv(SV *const sv)','name' => 'sv_2nv'},'SvREFCNT_inc_simple_NN' => {'text' => 'Same as SvREFCNT_inc_simple, but can only be used if you know I<sv>
...
false.
	...
	char*	sv_pvutf8n_force(SV *const sv, STRLEN *const lp)','name' => 'sv_pvutf8n_force'},'savepv' => {'text' => 'Perl\'s version of C<strdup()>. Returns a pointer to a newly allocated
...
be freed with the C<Safefree()> function.
	...
	void	SvNV_set(SV* sv, NV val)','name' => 'SvNV_set'},'bytes_from_utf8' => {'text' => 'Converts a string C<s> of length C<len> from UTF-8 into native byte encoding.
...
removed without notice.
	...
	U8*	bytes_from_utf8(const U8 *s, STRLEN *len, bool *is_utf8)','name' => 'bytes_from_utf8'},'newAV' => {'text' => 'Creates a new AV.  The reference count is set to 1.
...
for a version which guarantees to evaluate sv only once.
	...
	UV	SvUV(SV* sv)','name' => 'SvUV'},'PERL_SYS_INIT' => {'text' => 'Provides system-specific tune up of the C runtime environment necessary to
run Perl interpreters. This should be called only once, before creating
any Perl interpreters.

	void	PERL_SYS_INIT(int argc, char** argv)','name' => 'PERL_SYS_INIT'},'perl_run' => {'text' => 'Tells a Perl interpreter to run.  See L<perlembed>.

	int	perl_run(PerlInterpreter *my_perl)','name' => 'perl_run'},'vstringify' => {'text' => 'In order to maintain maximum compatibility with earlier versions
...
and C<sv_catsv_nomg> are implemented in terms of this function.
	...
	void	sv_catsv_flags(SV *const dsv, SV *const ssv, const I32 flags)','name' => 'sv_catsv_flags'},'sv_insert_flags' => {'text' => 'Same as C<sv_insert>, but the extra C<flags> are passed the C<SvPV_force_flags> that applies to C<bigstr>.
...
instead use an in-line version.
	...
	I32	sv_true(SV *const sv)','name' => 'sv_true'},'sv_derived_from' => {'text' => 'Returns a boolean indicating whether the SV is derived from the specified class
...
C<klen> is the length of the key.
	...
	bool	hv_exists(HV *hv, const char *key, I32 klen)','name' => 'hv_exists'},'dSP' => {'text' => 'Declares a local copy of perl\'s stack pointer for the XSUB, available via
the C<SP> macro.  See C<SP>.

		dSP;','name' => 'dSP'},'Nullsv' => {'text' => 'Null SV pointer. (No longer available when C<PERL_CORE> is defined.)','name' => 'Nullsv'},'mg_length' => {'text' => 'Report on the SV\'s length.  See C<sv_magic>.
...
is to be expected. (For information only--not to be used).','name' => 'HEf_SVKEY'},'SvOOK_offset' => {'text' => 'Reads into I<len> the offset from SvPVX back to the true start of the
...
Evaluates I<sv> more than once. Sets I<len> to 0 if C<SvOOK(sv)> is false.
	...
	void	SvOOK_offset(NN SV*sv, STRLEN len)','name' => 'SvOOK_offset'},'utf8_length' => {'text' => 'Return the length of the UTF-8 char encoded string C<s> in characters.
Stops at C<e> (inclusive).  If C<e E<lt> s> or if the scan would end
up past C<e>, croaks.

	STRLEN	utf8_length(const U8* s, const U8 *e)','name' => 'utf8_length'},'SvTAINTED_on' => {'text' => 'Marks an SV as tainted if tainting is enabled.
...
Usually used via one of its frontends C<sv_vcatpvf> and C<sv_vcatpvf_mg>.
	...
	void	sv_vcatpvfn(SV *const sv, const char *const pat, const STRLEN patlen, va_list *const args, SV **const svargs, const I32 svmax, bool *const maybe_tainted)','name' => 'sv_vcatpvfn'},'strLT' => {'text' => 'Test two strings to see if the first, C<s1>, is less than the second,
...
array is empty.
	...
	SV*	av_shift(AV *av)','name' => 'av_shift'},'sv_pvutf8n' => {'text' => 'A private implementation of the C<SvPVutf8> macro for compilers
which can\'t cope with complex macro expressions. Always use the macro
instead.

	char*	sv_pvutf8n(SV *sv, STRLEN *lp)','name' => 'sv_pvutf8n'},'newSV' => {'text' => 'Creates a new SV.  A non-zero C<len> parameter indicates the number of
...
Usually accessed via the C<SvPVutf8> macro.
	...
	char*	sv_2pvutf8(SV *const sv, STRLEN *const lp)','name' => 'sv_2pvutf8'},'sv_nounlocking' => {'text' => 'Dummy routine which "unlocks" an SV when there is no locking module present.
Exists to avoid test for a NULL function pointer and because it could
potentially warn under some level of strict-ness.
...
This is usually handled automatically by C<xsubpp> by calling C<dXSARGS>.
		...
		dITEMS;','name' => 'dITEMS'},'lex_unstuff' => {'text' => 'Discards text about to be lexed, from L</PL_parser-E<gt>bufptr> up to
...
removed without notice.
	...
	void	lex_unstuff(char *ptr)','name' => 'lex_unstuff'},'SvUV_set' => {'text' => 'Set the value of the UV pointer in sv to val.  See C<SvIV_set>.
...
optimise.
	...
	void	sv_setiv_mg(SV *const sv, const IV i)','name' => 'sv_setiv_mg'},'PUTBACK' => {'text' => 'Closing bracket for XSUB arguments.  This is usually handled by C<xsubpp>.
...
Normally used via the C<SvUV(sv)> and C<SvUVx(sv)> macros.
	...
	UV	sv_2uv_flags(SV *const sv, const I32 flags)','name' => 'sv_2uv_flags'},'SvNOK' => {'text' => 'Returns a U32 value indicating whether the SV contains a double.
...
usually end up here too.
	...
	char*	sv_2pv_flags(SV *const sv, STRLEN *const lp, const I32 flags)','name' => 'sv_2pv_flags'},'SvTAINTED_off' => {'text' => 'Untaints an SV. Be I<very> careful with this routine, as it short-circuits
...
of the typeglob that PL_defoutgv points to is decreased by one.
	...
	void	setdefout(GV* gv)','name' => 'setdefout'},'require_pv' => {'text' => 'Tells Perl to C<require> the file named by the string argument.  It is
...
You generally want to use the C<SvUPGRADE> macro wrapper. See also C<svtype>.
	...
	void	sv_upgrade(SV *const sv, svtype new_type)','name' => 'sv_upgrade'},'strEQ' => {'text' => 'Test two strings to see if they are equal.  Returns true or false.
...
information on how to use this function on tied hashes.
	...
	SV**	hv_store(HV *hv, const char *key, I32 klen, SV *val, U32 hash)','name' => 'hv_store'},'isUPPER' => {'text' => 'Returns a boolean indicating whether the C C<char> is a US-ASCII (Basic Latin)
uppercase character.

	bool	isUPPER(char ch)','name' => 'isUPPER'},'lex_peek_unichar' => {'text' => 'Looks ahead one (Unicode) character in the text currently being lexed.
...
removed without notice.
	...
	void	sv_setsv_mg(SV *const dstr, SV *const sstr)','name' => 'sv_setsv_mg'},'hv_fetch_ent' => {'text' => 'Returns the hash entry which corresponds to the specified key in the hash.
...
information on how to use this function on tied hashes.
	...
	HE*	hv_fetch_ent(HV *hv, SV *keysv, I32 lval, U32 hash)','name' => 'hv_fetch_ent'},'isLOWER' => {'text' => 'Returns a boolean indicating whether the C C<char> is a US-ASCII (Basic Latin)
lowercase character.
...
returned and retlen is set, if possible, to -1.
	...
	UV	utf8_to_uvchr(const U8 *s, STRLEN *retlen)','name' => 'utf8_to_uvchr'},'nothreadhook' => {'text' => 'Stub that provides thread hook for perl_destruct when there are
no threads.

	int	nothreadhook()','name' => 'nothreadhook'},'bytes_to_utf8' => {'text' => 'Converts a string C<s> of length C<len> from the native encoding into UTF-8.
...
cope with complex macro expressions. Always use the macro instead.
	...
	char*	sv_pvn(SV *sv, STRLEN *lp)','name' => 'sv_pvn'},'sv_setref_iv' => {'text' => 'Copies an integer into a new SV, optionally blessing the SV.  The C<rv>
...
wrapper instead.
	...
	int	sv_backoff(SV *const sv)','name' => 'sv_backoff'},'sv_catsv_nomg' => {'text' => 'Like C<sv_catsv> but doesn\'t process magic.
...
SV is set to 1.
	...
	SV*	newSViv(const IV i)','name' => 'newSViv'},'sv_untaint' => {'text' => 'Untaint an SV. Use C<SvTAINTED_off> instead.
	void	sv_untaint(SV *const sv)','name' => 'sv_untaint'},'sv_setpv' => {'text' => 'Copies a string into an SV.  The string must be null-terminated.  Does not
handle \'set\' magic.  See C<sv_setpv_mg>.

	void	sv_setpv(SV *const sv, const char *const ptr)','name' => 'sv_setpv'},'Newxz' => {'text' => 'The XSUB-writer\'s interface to the C C<malloc> function.  The allocated
...
valid UTF-8.  Handles \'get\' magic, but not \'set\' magic.  See C<sv_catpv_mg>.
	...
	void	sv_catpv(SV *const sv, const char* ptr)','name' => 'sv_catpv'},'sv_len' => {'text' => 'Returns the length of the string in the SV. Handles magic and type
coercion.  See also C<SvCUR>, which gives raw access to the xpv_cur slot.

	STRLEN	sv_len(SV *const sv)','name' => 'sv_len'},'SvPVbyte_nolen' => {'text' => 'Like C<SvPV_nolen>, but converts sv to byte representation first if necessary.

	char*	SvPVbyte_nolen(SV* sv)','name' => 'SvPVbyte_nolen'},'sv_pvutf8' => {'text' => 'Use the C<SvPVutf8_nolen> macro instead

	char*	sv_pvutf8(SV *sv)','name' => 'sv_pvutf8'},'SvRX' => {'text' => 'Convenience macro to get the REGEXP from a SV. This is approximately
...
NULL will be returned if a REGEXP* is not found.
	...
	REGEXP *	SvRX(SV *sv)','name' => 'SvRX'},'SVt_PVAV' => {'text' => 'Type flag for arrays.  See C<svtype>.','name' => 'SVt_PVAV'},'LEAVE' => {'text' => 'Closing bracket on a callback.  See C<ENTER> and L<perlcall>.
	...
	void	SvSetMagicSV_nosteal(SV* dsv, SV* ssv)','name' => 'SvSetMagicSV_nosteal'},'PL_keyword_plugin' => {'text' => 'Function pointer, pointing at a function used to handle extended keywords.
...
NOTE: this function is experimental and may change or be
removed without notice.','name' => 'PL_keyword_plugin'},'hv_delete_ent' => {'text' => 'Deletes a key/value pair in the hash.  The value SV is removed from the
hash and returned to the caller.  The C<flags> value will normally be zero;
if set to G_DISCARD then NULL will be returned.  C<hash> can be a valid
precomputed hash value, or 0 to ask for it to be computed.

	SV*	hv_delete_ent(HV *hv, SV *keysv, I32 flags, U32 hash)','name' => 'hv_delete_ent'},'CLASS' => {'text' => 'Variable which is setup by C<xsubpp> to indicate the 
...
C<SvPV_force> and C<SvPV_force_nomg>
	...
	char*	sv_pvn_force_flags(SV *const sv, STRLEN *const lp, const I32 flags)','name' => 'sv_pvn_force_flags'},'HeKLEN' => {'text' => 'If this is negative, and amounts to C<HEf_SVKEY>, it indicates the entry
...
(but note, as explained above, that there may be more.)
	...
	UV	to_utf8_title(const U8 *p, U8* ustrp, STRLEN *lenp)','name' => 'to_utf8_title'},'PUSHn' => {'text' => 'Push a double onto the stack.  The stack must have room for this element.
Handles \'set\' magic.  Uses C<TARG>, so C<dTARGET> or C<dXSTARG> should be
called to declare it.  Do not call multiple C<TARG>-oriented macros to
return lists from XSUB\'s - see C<mPUSHn> instead.  See also C<XPUSHn> and
C<mXPUSHn>.

	void	PUSHn(NV nv)','name' => 'PUSHn'},'mPUSHi' => {'text' => 'Push an integer onto the stack.  The stack must have room for this element.
Does not use C<TARG>.  See also C<PUSHi>, C<mXPUSHi> and C<XPUSHi>.

	void	mPUSHi(IV iv)','name' => 'mPUSHi'},'SvIV' => {'text' => 'Coerces the given SV to an integer and returns it. See C<SvIVx> for a
...
potentially warn under some level of strict-ness.
	...
	void	sv_nosharing(SV *sv)','name' => 'sv_nosharing'},'is_ascii_string' => {'text' => 'Returns true if first C<len> bytes of the given string are ASCII (i.e. none
...
will have a reference count of 1, and the RV will be returned.
	...
	SV*	sv_setref_uv(SV *const rv, const char *const classname, const UV uv)','name' => 'sv_setref_uv'},'getcwd_sv' => {'text' => 'Fill the sv with current working directory
...
then.
	...
	char*	fbm_instr(unsigned char* big, unsigned char* bigend, SV* littlestr, U32 flags)','name' => 'fbm_instr'},'sv_setpvf' => {'text' => 'Works like C<sv_catpvf> but copies the text into the SV instead of
appending it.  Does not handle \'set\' magic.  See C<sv_setpvf_mg>.

	void	sv_setpvf(SV *const sv, const char *const pat, ...)','name' => 'sv_setpvf'},'XPUSHn' => {'text' => 'Push a double onto the stack, extending the stack if necessary.  Handles
\'set\' magic.  Uses C<TARG>, so C<dTARGET> or C<dXSTARG> should be called to
declare it.  Do not call multiple C<TARG>-oriented macros to return lists
...
When you replace this variable, it is considered a good practice to store the possibly previously installed hook and that you recall it inside your own.
	...
	Perl_ophook_t	PL_opfreehook','name' => 'PL_opfreehook'},'SVt_PV' => {'text' => 'Pointer type flag for scalars.  See C<svtype>.','name' => 'SVt_PV'},'call_sv' => {'text' => 'Performs a callback to the Perl sub whose name is in the SV.  See
L<perlcall>.
...
type coercion.
	...
	void	sv_pos_u2b(SV *const sv, I32 *const offsetp, I32 *const lenp)','name' => 'sv_pos_u2b'},'PERL_SYS_INIT3' => {'text' => 'Provides system-specific tune up of the C runtime environment necessary to
run Perl interpreters. This should be called only once, before creating
any Perl interpreters.
...
to force this SV to be interpreted as an "extended" version.
	...
	SV*	upg_version(SV *ver, bool qv)','name' => 'upg_version'},'POPl' => {'text' => 'Pops a long off the stack.

	long	POPl','name' => 'POPl'},'HeSVKEY_set' => {'text' => 'Sets the key to a given C<SV*>, taking care to set the appropriate flags to
indicate the presence of an C<SV*> key, and returns the same
C<SV*>.

	SV*	HeSVKEY_set(HE* he, SV* sv)','name' => 'HeSVKEY_set'},'SPAGAIN' => {'text' => 'Refetch the stack pointer.  Used after a callback.  See L<perlcall>.

		SPAGAIN;','name' => 'SPAGAIN'},'scan_version' => {'text' => 'Returns a pointer to the next character after the parsed
...
it doesn\'t.
	...
	const char*	scan_version(const char *s, SV *rv, bool qv)','name' => 'scan_version'},'SvIOKp' => {'text' => 'Returns a U32 value indicating whether the SV contains an integer.  Checks
...
contained within the RV.
	...
	SV*	vnumify(SV *vs)','name' => 'vnumify'},'XST_mUNDEF' => {'text' => 'Place C<&PL_sv_undef> into the specified position C<pos> on the
stack.
...
statement boundaries.  See also C<sv_newmortal> and C<sv_2mortal>.
	...
	SV*	sv_mortalcopy(SV *const oldsv)','name' => 'sv_mortalcopy'},'sv_catpvn_nomg' => {'text' => 'Like C<sv_catpvn> but doesn\'t process magic.
...
pair.
	...
	SV*	newSVpvs_flags(const char* s, U32 flags)','name' => 'newSVpvs_flags'},'PL_parser-E<gt>bufptr' => {'text' => 'Points to the current position of lexing inside the lexer buffer.
...
of the SV is unaffected.
	...
	SV*	sv_bless(SV *const sv, HV *const stash)','name' => 'sv_bless'},'sv_isa' => {'text' => 'Returns a boolean indicating whether the SV is blessed into the specified
...
returned and retlen is set, if possible, to -1.
	...
	UV	utf8_to_uvuni(const U8 *s, STRLEN *retlen)','name' => 'utf8_to_uvuni'},'sv_2io' => {'text' => 'Using various gambits, try to get an IO from an SV: the IO slot if its a
...
SV if the hek is NULL.
	...
	SV*	newSVhek(const HEK *const hek)','name' => 'newSVhek'},'isALPHA' => {'text' => 'Returns a boolean indicating whether the C C<char> is a US-ASCII (Basic Latin)
alphabetic character.

	bool	isALPHA(char ch)','name' => 'isALPHA'},'Nullhv' => {'text' => 'Null HV pointer.

(deprecated - use C<(HV *)NULL> instead)','name' => 'Nullhv'},'av_fill' => {'text' => 'Set the highest index in the array to the given number, equivalent to
Perl\'s C<$#array = $fill;>.
...
the same as C<av_clear(av)>.
	...
	void	av_fill(AV *av, I32 fill)','name' => 'av_fill'},'SvREFCNT_inc' => {'text' => 'Increments the reference count of the given SV.
...
number may use \'_\' characters to separate digits.
	...
	UV	grok_bin(const char* start, STRLEN* len_p, I32* flags, NV *result)','name' => 'grok_bin'},'perl_free' => {'text' => 'Releases a Perl interpreter.  See L<perlembed>.
	...
	MAGIC*	mg_find(const SV* sv, int type)','name' => 'mg_find'},'SvNOK_on' => {'text' => 'Tells an SV that it is a double.
...
Does not use C<TARG>.  See also C<XPUSHs>, C<PUSHmortal> and C<PUSHs>.
	...
	void	XPUSHmortal()','name' => 'XPUSHmortal'},'PL_parser-E<gt>linestr' => {'text' => 'Buffer scalar containing the chunk currently under consideration of the
...
NOTE: this function is experimental and may change or be
removed without notice.','name' => 'PL_parser-E<gt>linestr'},'SvVOK' => {'text' => 'Returns a boolean indicating whether the SV contains a v-string.

	bool	SvVOK(SV* sv)','name' => 'SvVOK'},'newSVpvn_share' => {'text' => 'Creates a new SV with its SvPVX_const pointing to a shared string in the string
...
SvPVX_const == HeKEY and hash lookup will avoid string compare.
	...
	void	sv_setpvf_mg(SV *const sv, const char *const pat, ...)','name' => 'sv_setpvf_mg'},'SvPOKp' => {'text' => 'Returns a U32 value indicating whether the SV contains a character string.
...
Handles \'get\' magic, but not \'set\' magic.  See C<sv_catpvn_mg>.
	...
	void	sv_catpvn(SV *dsv, const char *sstr, STRLEN len)','name' => 'sv_catpvn'},'sv_setref_pv' => {'text' => 'Copies a pointer into a new SV, optionally blessing the SV.  The C<rv>
argument will be upgraded to an RV.  That RV will be modified to point to
...
removed without notice.
	...
	bool	sv_utf8_downgrade(SV *const sv, const bool fail_ok)','name' => 'sv_utf8_downgrade'},'SvNIOKp' => {'text' => 'Returns a U32 value indicating whether the SV contains a number, integer or
...
freeing any remaining Perl interpreters.
	...
	void	PERL_SYS_TERM()','name' => 'PERL_SYS_TERM'},'utf8_hop' => {'text' => 'Return the UTF-8 pointer C<s> displaced by C<off> characters, either
forward or backward.
...
on the first byte of character or just after the last byte of a character.
	...
	U8*	utf8_hop(const U8 *s, I32 off)','name' => 'utf8_hop'},'sv_len_utf8' => {'text' => 'Returns the number of characters in the string in an SV, counting wide
UTF-8 bytes as a single character. Handles magic and type coercion.

	STRLEN	sv_len_utf8(SV *const sv)','name' => 'sv_len_utf8'},'sv_setref_pvn' => {'text' => 'Copies a string into a new SV, optionally blessing the SV.  The length of the
...
Note that C<sv_setref_pv> copies the pointer while this copies the string.
	...
	SV*	sv_setref_pvn(SV *const rv, const char *const classname, const char *const pv, const STRLEN n)','name' => 'sv_setref_pvn'},'SVt_PVMG' => {'text' => 'Type flag for blessed scalars.  See C<svtype>.','name' => 'SVt_PVMG'},'PUSHp' => {'text' => 'Push a string onto the stack.  The stack must have room for this element.
...
See Hash::Util::lock_keys() for an example of its use.
	...
	void	hv_clear_placeholders(HV *hv)','name' => 'hv_clear_placeholders'},'mXPUSHi' => {'text' => 'Push an integer onto the stack, extending the stack if necessary.
...
settings.
	...
	char*	sv_collxfrm(SV *const sv, STRLEN *const nxp)','name' => 'sv_collxfrm'},'ZeroD' => {'text' => 'Like C<Zero> but returns dest. Useful for encouraging compilers to tail-call
optimise.
...
converted into version objects.
	...
	int	vcmp(SV *lhv, SV *rhv)','name' => 'vcmp'},'PL_modglobal' => {'text' => 'C<PL_modglobal> is a general purpose, interpreter global HV for use by
...
NOTE: this function is experimental and may change or be
removed without notice.','name' => 'PL_parser-E<gt>bufend'},'pv_escape' => {'text' => 'Escapes at most the first "count" chars of pv and puts the results into
...
incremented.
	...
	void	sv_setpviv_mg(SV *const sv, const IV iv)','name' => 'sv_setpviv_mg'},'SvUVx' => {'text' => 'Coerces the given SV to an unsigned integer and returns it. Guarantees to
C<sv> only once. Only use this if C<sv> is an expression with side effects,
...
sv_true() or its macro equivalent.
	...
	bool	sv_2bool(SV *const sv)','name' => 'sv_2bool'},'dORIGMARK' => {'text' => 'Saves the original stack mark for the XSUB.  See C<ORIGMARK>.

		dORIGMARK;','name' => 'dORIGMARK'},'G_VOID' => {'text' => 'Used to indicate void context.  See C<GIMME_V> and L<perlcall>.','name' => 'G_VOID'},'lex_grow_linestr' => {'text' => 'Reallocates the lexer buffer (L</PL_parser-E<gt>linestr>) to accommodate
...
removed without notice.
	...
	char *	lex_grow_linestr(STRLEN len)','name' => 'lex_grow_linestr'},'sv_newmortal' => {'text' => 'Creates a new null SV which is mortal.  The reference count of the SV is
...
removed without notice.
	...
	void	lex_read_space(U32 flags)','name' => 'lex_read_space'},'SvOOK' => {'text' => 'Returns a U32 indicating whether the pointer to the string buffer is offset.
...
the scalar\'s value cannot change unless written to.
	...
	U32	SvGAMAGIC(SV* sv)','name' => 'SvGAMAGIC'},'gv_try_downgrade' => {'text' => 'If C<gv> is a typeglob containing only a constant sub, and is only
...
to be a constant sub to which a proper reference is no longer required.
	...
	void	gv_try_downgrade(GV* gv)','name' => 'gv_try_downgrade'},'svtype' => {'text' => 'An enum of flags for Perl types.  These are found in the file B<sv.h>
...
import list is required.
	...
	void	load_module(U32 flags, SV* name, SV* ver, ...)','name' => 'load_module'},'get_hv' => {'text' => 'Returns the HV of the specified Perl hash.  C<flags> are passed to
...
NOTE: the perl_ form of this function is deprecated.
	...
	HV*	get_hv(const char *name, I32 flags)','name' => 'get_hv'},'hv_clear' => {'text' => 'Clears a hash, making it empty.

	void	hv_clear(HV *hv)','name' => 'hv_clear'},'PoisonNew' => {'text' => 'PoisonWith(0xAB) for catching access to allocated but uninitialized memory.
...
cast.
	...
	void	sv_catpvf_mg(SV *const sv, const char *const pat, ...)','name' => 'sv_catpvf_mg'},'PUSHu' => {'text' => 'Push an unsigned integer onto the stack.  The stack must have room for this
element.  Handles \'set\' magic.  Uses C<TARG>, so C<dTARGET> or C<dXSTARG>
should be called to declare it.  Do not call multiple C<TARG>-oriented
macros to return lists from XSUB\'s - see C<mPUSHu> instead.  See also
C<XPUSHu> and C<mXPUSHu>.

	void	PUSHu(UV uv)','name' => 'PUSHu'},'get_sv' => {'text' => 'Returns the SV of the specified Perl scalar.  C<flags> are passed to
C<gv_fetchpv>. If C<GV_ADD> is set and the
Perl variable does not exist then it will be created.  If C<flags> is zero
and the variable does not exist then NULL is returned.
...
NOTE: the perl_ form of this function is deprecated.
	...
	SV*	get_sv(const char *name, I32 flags)','name' => 'get_sv'},'SvPOK_off' => {'text' => 'Unsets the PV status of an SV.
...
(Uses C<sv_setsv>).
	...
	void	sv_setpv_mg(SV *const sv, const char *const ptr)','name' => 'sv_setpv_mg'},'lex_next_chunk' => {'text' => 'Reads in the next chunk of text to be lexed, appending it to
...
and C<sv_mortalcopy>.
	...
	SV*	sv_2mortal(SV *const sv)','name' => 'sv_2mortal'},'XCPT_TRY_END' => {'text' => 'Ends a try block.  See L<perlguts/"Exception Handling">.','name' => 'XCPT_TRY_END'},'av_undef' => {'text' => 'Undefines the array.  Frees the memory used by the array itself.

	void	av_undef(AV *av)','name' => 'av_undef'},'HeKEY' => {'text' => 'Returns the actual pointer stored in the key slot of the hash entry. The
...
flags are not used. This call should not be used; use packlist instead.
	...
	void	pack_cat(SV *cat, const char *pat, const char *patend, SV **beglist, SV **endlist, SV ***next_in_list, U32 flags)','name' => 'pack_cat'},'sv_cmp' => {'text' => 'Compares the strings in two SVs.  Returns -1, 0, or 1 indicating whether the
string in C<sv1> is less than, equal to, or greater than the string in
C<sv2>. Is UTF-8 and \'use bytes\' aware, handles get magic, and will
coerce its args to strings if necessary.  See also C<sv_cmp_locale>.

	I32	sv_cmp(SV *const sv1, SV *const sv2)','name' => 'sv_cmp'},'SvEND' => {'text' => 'Returns a pointer to the last character in the string which is in the SV.
...
with flags set to 0.
	...
	void	sv_force_normal_flags(SV *const sv, const U32 flags)','name' => 'sv_force_normal_flags'},'sv_setsv_flags' => {'text' => 'Copies the contents of the source SV C<ssv> into the destination SV
...
copy-ish functions and macros use this underneath.
	...
	void	sv_setsv_flags(SV *dstr, SV *sstr, const I32 flags)','name' => 'sv_setsv_flags'},'is_utf8_char' => {'text' => 'Tests if some arbitrary number of bytes begins in a valid UTF-8
character.  Note that an INVARIANT (i.e. ASCII on non-EBCDIC machines)
character is a valid UTF-8 character.  The actual number of bytes in the UTF-8
character will be returned if it is valid, otherwise 0.

	STRLEN	is_utf8_char(const U8 *s)','name' => 'is_utf8_char'},'Copy' => {'text' => 'The XSUB-writer\'s interface to the C C<memcpy> function.  The C<src> is the
...
whitespace.
	...
	void	Safefree(void* ptr)','name' => 'Safefree'},'PL_parser-E<gt>linestart' => {'text' => 'Points to the start of the current line inside the lexer buffer.
...
NOTE: this function is experimental and may change or be
removed without notice.','name' => 'PL_parser-E<gt>linestart'},'gv_stashpvn' => {'text' => 'Returns a pointer to the stash for a specified package.  The C<namelen>
...
is set to 1.
	...
	SV*	newSV_type(const svtype type)','name' => 'newSV_type'},'sv_setsv_nomg' => {'text' => 'Like C<sv_setsv> but doesn\'t process magic.
...
contain an C<SV*> key.
	...
	SV*	HeSVKEY(HE* he)','name' => 'HeSVKEY'},'new_version' => {'text' => 'Returns a new version object based on the passed in SV:
...
want to upgrade the SV.
	...
	SV*	new_version(SV *ver)','name' => 'new_version'},'sv_pvbyten' => {'text' => 'A private implementation of the C<SvPVbyte> macro for compilers
which can\'t cope with complex macro expressions. Always use the macro
instead.

	char*	sv_pvbyten(SV *sv, STRLEN *lp)','name' => 'sv_pvbyten'},'Nullav' => {'text' => 'Null AV pointer.

(deprecated - use C<(AV *)NULL> instead)','name' => 'Nullav'},'sv_copypv' => {'text' => 'Copies a stringified representation of the source SV into the
...
Returns a pointer to the prettified text as held by dsv.
	...
	void	sv_setnv_mg(SV *const sv, const NV num)','name' => 'sv_setnv_mg'},'hv_iternextsv' => {'text' => 'Performs an C<hv_iternext>, C<hv_iterkey>, and C<hv_iterval> in one
operation.

	SV*	hv_iternextsv(HV *hv, char **key, I32 *retlen)','name' => 'hv_iternextsv'},'utf8_to_bytes' => {'text' => 'Converts a string C<s> of length C<len> from UTF-8 into native byte encoding.
...
removed without notice.
	...
	U8*	utf8_to_bytes(U8 *s, STRLEN *len)','name' => 'utf8_to_bytes'},'lex_read_to' => {'text' => 'Consume text in the lexer buffer, from L</PL_parser-E<gt>bufptr> up
...
removed without notice.
	...
	char*	sv_pvbyten_force(SV *const sv, STRLEN *const lp)','name' => 'sv_pvbyten_force'},'uvuni_to_utf8_flags' => {'text' => 'Adds the UTF-8 representation of the Unicode codepoint C<uv> to the end
...
is a lexical $_ in scope.','name' => 'UNDERBAR'},'pad_sv' => {'text' => 'Get the value at offset po in the current pad.
...
Usually accessed via the C<SvPVutf8_nolen> macro.
	...
	char*	sv_2pvutf8_nolen(SV* sv)','name' => 'sv_2pvutf8_nolen'},'SVt_NV' => {'text' => 'Double type flag for scalars.  See C<svtype>.','name' => 'SVt_NV'},'mro_get_linear_isa' => {'text' => 'Returns either C<mro_get_linear_isa_c3> or
...
more information on how to use this function on tied arrays. 
	...
	SV**	av_fetch(AV *av, I32 key, I32 lval)','name' => 'av_fetch'},'gv_fetchmeth_autoload' => {'text' => 'Same as gv_fetchmeth(), but looks for autoloaded subroutines too.
...
C<hv_iterkey>.
	...
	const char*	sv_reftype(const SV *const sv, const int ob)','name' => 'sv_reftype'},'sv_pos_b2u' => {'text' => 'Converts the value pointed to by offsetp from a count of bytes from the
start of the string, to a count of the equivalent number of UTF-8 chars.
Handles magic and type coercion.

	void	sv_pos_b2u(SV *const sv, I32 *const offsetp)','name' => 'sv_pos_b2u'},'SvREFCNT_inc_simple_void_NN' => {'text' => 'Same as SvREFCNT_inc, but can only be used if you don\'t need the return
value, and you know that I<sv> is not NULL.  The macro doesn\'t need
to return a meaningful value, or check for NULLness, so it\'s smaller
and faster.
...
array is C<av_len(av) + 1>.  Returns -1 if the array is empty.
	...
	I32	av_len(AV *av)','name' => 'av_len'},'SVt_IV' => {'text' => 'Integer type flag for scalars.  See C<svtype>.','name' => 'SVt_IV'},'SvIsCOW' => {'text' => 'Returns a boolean indicating whether the SV is Copy-On-Write. (either shared
...
(but note, as explained above, that there may be more.)
	...
	UV	to_utf8_fold(const U8 *p, U8* ustrp, STRLEN *lenp)','name' => 'to_utf8_fold'},'to_utf8_upper' => {'text' => 'Convert the UTF-8 encoded character at p to its uppercase version and
store that in UTF-8 in ustrp and its length in bytes in lenp.  Note
...
(but note, as explained above, that there may be more.)
	...
	UV	to_utf8_upper(const U8 *p, U8* ustrp, STRLEN *lenp)','name' => 'to_utf8_upper'},'gv_stashpv' => {'text' => 'Returns a pointer to the stash for a specified package.  Uses C<strlen> to
...
Usually used via its frontend C<sv_catpvf_mg>.
	...
	void	sv_vcatpvf_mg(SV *const sv, const char *const pat, va_list *const args)','name' => 'sv_vcatpvf_mg'},'SvPVutf8x_force' => {'text' => 'Like C<SvPV_force>, but converts sv to utf8 first if necessary.
...
computed.
	...
	bool	hv_exists_ent(HV *hv, SV *keysv, U32 hash)','name' => 'hv_exists_ent'},'SvLEN_set' => {'text' => 'Set the actual length of the string which is in the SV.  See C<SvIV_set>.
...
(but note, as explained above, that there may be more.)
	...
	UV	to_utf8_lower(const U8 *p, U8* ustrp, STRLEN *lenp)','name' => 'to_utf8_lower'},'SvNIOK' => {'text' => 'Returns a U32 value indicating whether the SV contains a number, integer or
double.

	U32	SvNIOK(SV* sv)','name' => 'SvNIOK'},'mro_method_changed_in' => {'text' => 'Invalidates method caching on any child classes
...
and ocnt are not used. This call should not be used, use unpackstring instead.
	...
	I32	unpack_str(const char *pat, const char *patend, const char *s, const char *strbeg, const char *strend, char **new_s, I32 ocnt, U32 flags)','name' => 'unpack_str'},'newSVpvf' => {'text' => 'Creates a new SV and initializes it with the string formatted like
C<sprintf>.

	SV*	newSVpvf(const char *const pat, ...)','name' => 'newSVpvf'},'Newxc' => {'text' => 'The XSUB-writer\'s interface to the C C<malloc> function, with
...
instead.
	...
	SV*	sv_newref(SV *const sv)','name' => 'sv_newref'},'perl_destruct' => {'text' => 'Shuts down a Perl interpreter.  See L<perlembed>.

	int	perl_destruct(PerlInterpreter *my_perl)','name' => 'perl_destruct'},'XSRETURN_IV' => {'text' => 'Return an integer from an XSUB immediately.  Uses C<XST_mIV>.
...
The pointer to the PV of the dsv is returned.
	...
	char*	sv_uni_display(SV *dsv, SV *ssv, STRLEN pvlim, UV flags)','name' => 'sv_uni_display'},'sv_vsetpvf' => {'text' => 'Works like C<sv_vcatpvf> but copies the text into the SV instead of
appending it.  Does not handle \'set\' magic.  See C<sv_vsetpvf_mg>.

Usually used via its frontend C<sv_setpvf>.

	void	sv_vsetpvf(SV *const sv, const char *const pat, va_list *const args)','name' => 'sv_vsetpvf'},'newSVpvs' => {'text' => 'Like C<newSVpvn>, but takes a literal string instead of a string/length pair.

	SV*	newSVpvs(const char* s)','name' => 'newSVpvs'},'SvREFCNT_dec' => {'text' => 'Decrements the reference count of the given SV.

	void	SvREFCNT_dec(SV* sv)','name' => 'SvREFCNT_dec'},'XCPT_CATCH' => {'text' => 'Introduces a catch block.  See L<perlguts/"Exception Handling">.','name' => 'XCPT_CATCH'},'sv_setsv' => {'text' => 'Copies the contents of the source SV C<ssv> into the destination SV
C<dsv>.  The source SV may be destroyed if it is mortal, so don\'t use this
function if the source SV needs to be reused. Does not handle \'set\' magic.
Loosely speaking, it performs a copy-by-value, obliterating any previous
content of the destination.
...
C<SvSetMagicSV_nosteal>.
	...
	void	sv_setsv(SV *dstr, SV *sstr)','name' => 'sv_setsv'},'XST_mYES' => {'text' => 'Place C<&PL_sv_yes> into the specified position C<pos> on the
stack.
...
The value is stored in a new mortal SV.
	...
	void	XST_mPV(int pos, char* str)','name' => 'XST_mPV'},'vverify' => {'text' => 'Validates that the SV contains a valid version object.
...
See L<perlguts/"Exception Handling">.
	...
	bool	sv_tainted(SV *const sv)','name' => 'sv_tainted'},'sv_grow' => {'text' => 'Expands the character buffer in the SV.  If necessary, uses C<sv_unref> and
...
contained within the RV.
	...
	SV*	vnormal(SV *vs)','name' => 'vnormal'},'cv_const_sv' => {'text' => 'If C<cv> is a constant sub eligible for inlining. returns the constant
...
The reference count for the SV is set to 1.
	...
	SV*	newSVnv(const NV n)','name' => 'newSVnv'},'XPUSHs' => {'text' => 'Push an SV onto the stack, extending the stack if necessary.  Does not
...
efficient.
	...
	char*	HePV(HE* he, STRLEN len)','name' => 'HePV'},'pv_uni_display' => {'text' => 'Build to the scalar dsv a displayable version of the string spv,
...
The pointer to the PV of the dsv is returned.
	...
	char*	pv_uni_display(SV *dsv, const U8 *spv, STRLEN len, STRLEN pvlim, UV flags)','name' => 'pv_uni_display'},'SvGETMAGIC' => {'text' => 'Invokes C<mg_get> on an SV if it has \'get\' magic.  This macro evaluates its
argument more than once.
...
Individual members of C<PL_parser> have their own documentation.','name' => 'PL_parser'},'sv_inc' => {'text' => 'Auto-increment of the value in the SV, doing string to numeric conversion
if necessary. Handles \'get\' magic.

	void	sv_inc(SV *const sv)','name' => 'sv_inc'},'sv_usepvn' => {'text' => 'Tells an SV to use C<ptr> to find its string value. Implemented by
...
will be returned.
	...
	SV*	hv_delete(HV *hv, const char *key, I32 klen, I32 flags)','name' => 'hv_delete'},'sv_cat_decode' => {'text' => 'The encoding is assumed to be an Encode object, the PV of the ssv is
...
Returns TRUE if the terminator was found, else returns FALSE.
	...
	bool	sv_cat_decode(SV* dsv, SV *encoding, SV *ssv, int *offset, char* tstr, int tlen)','name' => 'sv_cat_decode'},'POP_MULTICALL' => {'text' => 'Closing bracket for a lightweight callback.
See L<perlcall/Lightweight Callbacks>.

		POP_MULTICALL;','name' => 'POP_MULTICALL'},'SvIV_set' => {'text' => 'Set the value of the IV pointer in sv to val.  It is possible to perform
...
Does not handle \'set\' magic.  See also C<sv_setiv_mg>.
	...
	void	sv_setiv(SV *const sv, const IV num)','name' => 'sv_setiv'},'sv_report_used' => {'text' => 'Dump the contents of all SVs not yet freed. (Debugging aid).
...
obtained from the GV with the C<GvCV> macro.
	...
	U32	SvIOK(SV* sv)','name' => 'SvIOK'},'sv_does' => {'text' => 'Returns a boolean indicating whether the SV performs a specific, named role.
The SV can be a Perl object or the name of a Perl class.

	bool	sv_does(SV* sv, const char *const name)','name' => 'sv_does'},'SvPV_force' => {'text' => 'Like C<SvPV> but will force the SV into containing just a string
...
number may use \'_\' characters to separate digits.
	...
	UV	grok_hex(const char* start, STRLEN* len_p, I32* flags, NV *result)','name' => 'grok_hex'},'hv_iternext_flags' => {'text' => 'Returns entries from a hash iterator.  See C<hv_iterinit> and C<hv_iternext>.
...
Usually used via one of its frontends C<sv_vsetpvf> and C<sv_vsetpvf_mg>.
	...
	void	sv_vsetpvfn(SV *const sv, const char *const pat, const STRLEN patlen, va_list *const args, SV **const svargs, const I32 svmax, bool *const maybe_tainted)','name' => 'sv_vsetpvfn'},'perl_parse' => {'text' => 'Tells a Perl interpreter to parse a Perl script.  See L<perlembed>.

	int	perl_parse(PerlInterpreter *my_perl, XSINIT_t xsinit, int argc, char** argv, char** env)','name' => 'perl_parse'},'to_utf8_case' => {'text' => 'The "p" contains the pointer to the UTF-8 string encoding
...
%utf8::ToLower.
	...
	UV	to_utf8_case(const U8 *p, U8* ustrp, STRLEN *lenp, SV **swashp, const char *normal, const char *special)','name' => 'to_utf8_case'},'av_clear' => {'text' => 'Clears an array, making it empty.  Does not free the memory used by the
array itself.
...
must then use C<av_store> to assign values to these new elements.
	...
	void	av_unshift(AV *av, I32 num)','name' => 'av_unshift'},'SvREFCNT_inc_NN' => {'text' => 'Same as SvREFCNT_inc, but can only be used if you know I<sv>
is not NULL.  Since we don\'t have to check the NULLness, it\'s faster
and smaller.
...
removed without notice.
	...
	bool	sv_utf8_decode(SV *const sv)','name' => 'sv_utf8_decode'},'hv_stores' => {'text' => 'Like C<hv_store>, but takes a literal string instead of a string/length pair
...
L<perlcall>.
	...
	STRLEN	sv_utf8_upgrade_nomg(SV *sv)','name' => 'sv_utf8_upgrade_nomg'},'Move' => {'text' => 'The XSUB-writer\'s interface to the C C<memmove> function.  The C<src> is the
source, C<dest> is the destination, C<nitems> is the number of items, and C<type> is
...
in terms of this function.
	...
	void	sv_catpvn_flags(SV *const dstr, const char *sstr, const STRLEN len, const I32 flags)','name' => 'sv_catpvn_flags'},'sv_pvn_force' => {'text' => 'Get a sensible string out of the SV somehow.
...
Normally used via the C<SvIV(sv)> and C<SvIVx(sv)> macros.
	...
	IV	sv_2iv_flags(SV *const sv, const I32 flags)','name' => 'sv_2iv_flags'},'newXS' => {'text' => 'Used by C<xsubpp> to hook up XSUBs as Perl subs.  I<filename> needs to be
static storage, as it is used directly as CvFILE(), without a copy being made.','name' => 'newXS'},'isALNUM' => {'text' => 'Returns a boolean indicating whether the C C<char> is a US-ASCII (Basic Latin)
alphanumeric character (including underscore) or digit.

	bool	isALNUM(char ch)','name' => 'isALNUM'},'my_snprintf' => {'text' => 'The C library C<snprintf> functionality, if available and
...
getting C<vsnprintf>.
	...
	int	my_snprintf(char *buffer, const Size_t len, const char *format, ...)','name' => 'my_snprintf'},'XCPT_RETHROW' => {'text' => 'Rethrows a previously caught exception.  See L<perlguts/"Exception Handling">.

		XCPT_RETHROW;','name' => 'XCPT_RETHROW'},'SvIOK_UV' => {'text' => 'Returns a boolean indicating whether the SV contains an unsigned integer.
...
if necessary. Handles \'get\' magic.
	...
	void	sv_dec(SV *const sv)','name' => 'sv_dec'},'strLE' => {'text' => 'Test two strings to see if the first, C<s1>, is less than or equal to the
...
Note that the final string may be up to 7 chars longer than pvlim.
	...
	char*	pv_display(SV *dsv, const char *pv, STRLEN cur, STRLEN len, STRLEN pvlim)','name' => 'pv_display'},'newSVpvn_flags' => {'text' => 'Creates a new SV and copies a string into it.  The reference count for the
...
C<newSVpvn_utf8()> is a convenience wrapper for this function, defined as
	...
	SV*	newSVpvn_flags(const char *const s, const STRLEN len, const U32 flags)','name' => 'newSVpvn_flags'},'sv_2pvbyte_nolen' => {'text' => 'Return a pointer to the byte-encoded representation of the SV.
May cause the SV to be downgraded from UTF-8 as a side-effect.

Usually accessed via the C<SvPVbyte_nolen> macro.

	char*	sv_2pvbyte_nolen(SV* sv)','name' => 'sv_2pvbyte_nolen'},'SvOK' => {'text' => 'Returns a U32 value indicating whether the value is defined. This is
...
will have a reference count of 1, and the RV will be returned.
	...
	SV*	sv_setref_nv(SV *const rv, const char *const classname, const NV nv)','name' => 'sv_setref_nv'},'uvchr_to_utf8' => {'text' => 'Adds the UTF-8 representation of the Native codepoint C<uv> to the end
of the string C<d>; C<d> should be have at least C<UTF8_MAXBYTES+1> free
bytes available. The return value is the pointer to the byte after the
end of the new character. In other words,
...
digit.
	...
	bool	isDIGIT(char ch)','name' => 'isDIGIT'},'lex_stuff_pvn' => {'text' => 'Insert characters into the lexer buffer (L</PL_parser-E<gt>linestr>),
immediately after the current lexing point (L</PL_parser-E<gt>bufptr>),
reallocating the buffer if necessary.  This means that lexing code that
runs later will see the characters as if they had appeared in the input.
It is not recommended to do this as part of normal parsing, and most
...
removed without notice.
	...
	void	lex_stuff_pvn(char *pv, STRLEN len, U32 flags)','name' => 'lex_stuff_pvn'},'eval_pv' => {'text' => 'Tells Perl to C<eval> the given string and return an SV* result.
...
Normally called via a wrapper macro C<SvREFCNT_dec>.
	...
	void	sv_free(SV *const sv)','name' => 'sv_free'},'SvPVbyte_force' => {'text' => 'Like C<SvPV_force>, but converts sv to byte representation first if necessary.
...
is empty.
	...
	SV*	av_pop(AV *av)','name' => 'av_pop'},'croak' => {'text' => 'This is the XSUB-writer\'s interface to Perl\'s C<die> function.
...
(deprecated - use C<(CV *)NULL> instead)','name' => 'Nullcv'},'lex_read_unichar' => {'text' => 'Reads the next (Unicode) character in the text currently being lexed.
...
removed without notice.
	...
	I32	lex_read_unichar(U32 flags)','name' => 'lex_read_unichar'},'lex_bufutf8' => {'text' => 'Indicates whether the octets in the lexer buffer
...
removed without notice.
	...
	bool	lex_bufutf8()','name' => 'lex_bufutf8'},'get_av' => {'text' => 'Returns the AV of the specified Perl array.  C<flags> are passed to
C<gv_fetchpv>. If C<GV_ADD> is set and the
Perl variable does not exist then it will be created.  If C<flags> is zero
and the variable does not exist then NULL is returned.
...
use the macro wrapper C<SvPV_nolen(sv)> instead.
	char*	sv_2pv_nolen(SV* sv)','name' => 'sv_2pv_nolen'},'MULTICALL' => {'text' => 'Make a lightweight callback. See L<perlcall/Lightweight Callbacks>.
	...
	void	PoisonFree(void* dest, int nitems, type)','name' => 'PoisonFree'},'toUPPER' => {'text' => 'Converts the specified character to uppercase.  Characters outside the
US-ASCII (Basic Latin) range are viewed as not having any case.
...
See also is_utf8_string_loclen() and is_utf8_string().
	...
	bool	is_utf8_string_loc(const U8 *s, STRLEN len, const U8 **p)','name' => 'is_utf8_string_loc'},'SAVETMPS' => {'text' => 'Opening bracket for temporaries on a callback.  See C<FREETMPS> and
L<perlcall>.
...
appending to the currently-stored string.
	...
	char*	sv_gets(SV *const sv, PerlIO *const fp, I32 append)','name' => 'sv_gets'},'ORIGMARK' => {'text' => 'The original stack mark for the XSUB.  See C<dORIGMARK>.','name' => 'ORIGMARK'},'SvPVutf8x' => {'text' => 'Like C<SvPV>, but converts sv to utf8 first if necessary.
...
The flags in C<lref> are passed to gv_fetchsv.
	...
	CV*	sv_2cv(SV* sv, HV **const st, GV **const gvp, const I32 lref)','name' => 'sv_2cv'},'SvPV_force_nomg' => {'text' => 'Like C<SvPV> but will force the SV into containing just a string
(C<SvPOK_only>).  You want force if you are going to update the C<SvPVX>
...
cope with complex macro expressions. Always use the macro instead.
	...
	UV	sv_uv(SV* sv)','name' => 'sv_uv'},'XCPT_TRY_START' => {'text' => 'Starts a try block.  See L<perlguts/"Exception Handling">.','name' => 'XCPT_TRY_START'},'sv_unref_flags' => {'text' => 'Unsets the RV status of the SV, and decrements the reference count of
whatever was being referenced by the RV.  This can almost be thought of
...
See C<SvROK_off>.
	...
	SV*	SvRV(SV* sv)','name' => 'SvRV'},'Nullch' => {'text' => 'Null character pointer. (No longer available when C<PERL_CORE> is defined.)','name' => 'Nullch'},'mg_copy' => {'text' => 'Copies the magic from one SV to another.  See C<sv_magic>.

	int	mg_copy(SV *sv, SV *nsv, const char *key, I32 klen)','name' => 'mg_copy'},'grok_oct' => {'text' => 'converts a string representing an octal number to numeric form.
...
C<len> bytes long.  If the C<s> argument is NULL the new SV will be undefined.
	...
	SV*	newSVpvn(const char *const s, const STRLEN len)','name' => 'newSVpvn'},'sv_magicext' => {'text' => 'Adds magic to an SV, upgrading it if necessary. Applies the
...
(This is now used as a subroutine by C<sv_magic>.)
	...
	MAGIC *	sv_magicext(SV *const sv, SV *const obj, const int how, const MGVTBL *const vtbl, const char *const name, const I32 namlen)','name' => 'sv_magicext'},'av_push' => {'text' => 'Pushes an SV onto the end of the array.  The array will grow automatically
to accommodate the addition. Like C<av_store>, this takes ownership of one
reference count.

	void	av_push(AV *av, SV *val)','name' => 'av_push'},'unpackstring' => {'text' => 'The engine implementing unpack() Perl function. C<unpackstring> puts the
extracted list items on the stack and returns the number of elements.
Issue C<PUTBACK> before and C<SPAGAIN> after the call to this function.

	I32	unpackstring(const char *pat, const char *patend, const char *s, const char *strend, U32 flags)','name' => 'unpackstring'},'POPn' => {'text' => 'Pops a double off the stack.
...
extended.
	...
	void	av_extend(AV *av, I32 key)','name' => 'av_extend'},'newRV_noinc' => {'text' => 'Creates an RV wrapper for an SV.  The reference count for the original
SV is B<not> incremented.

	SV*	newRV_noinc(SV *const sv)','name' => 'newRV_noinc'},'hv_iternext' => {'text' => 'Returns entries from a hash iterator.  See C<hv_iterinit>.
...
trigger the resource deallocation.
	...
	HE*	hv_iternext(HV *hv)','name' => 'hv_iternext'},'hv_iterinit' => {'text' => 'Prepares a starting point to traverse a hash table.  Returns the number of
