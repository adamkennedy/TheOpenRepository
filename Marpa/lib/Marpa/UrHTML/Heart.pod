...
Marpa::UrHTML::How_It_Works - The Dao of HTML
...
HTML, that is, determining an HTML
document's overall structure, has been considered
a difficult problem in the past.
This document outlines a straightforward,
natural and transparent method for parsing
arbitrary documents in the spirit of
the HTML family of standards.

=head1 THE BASIC IDEA

L<Marpa::UrHTML> combines two major components.
It uses what I will call a Wishful Thinking Parser and
a Wish-Fulfilling Tokenizer.

The Wishful Thinking Parser ("Wishful Parser" for short)
uses L<a grammar|/"APPENDIX: THE GRAMMAR">
that makes a convenient, but false assumption
about HTML.
The Wishful Parser assumes that all elements have start and end tags.
This assumption is not true of the kind of HTML actually found
out on the Web, which L<Marpa::UrHTML> will parse.
In fact,
this assumption is not even true of HTML strictly valid according to the
standards.

Paired with the Wishful Parser is a Wish-Granting Tokenizer,
(Wish-Granter for short).
It is the job of this Tokenizer to make the Wishful Parser's
false assumption about HTML look right.
The Wish-Granting Tokenizer
creates a virtual
document that makes the Wishful Parser's wishes come true.
This virtual document needs to be easily mapped to the
physical one.
...
But L<Marpa::UrHTML> takes a different approach.
...
picture.
L<Marpa::UrHTML> uses this approach to let each
of the two components handle what they do best.

Crucial to the workability of this method is that
L<Marpa> allows predictive lexing.
In predictive lexing the high-level parser is
able to tell the lexer
which tokens will allow the parse to proceed.
The combination of
predictive lexing and Earley's
is new with the L<Marpa> algorithm
and apparently something of a surprise.

=head1 THE WISH-GRANTING TOKENIZER
...
A Wish-Granting Tokenizer is not hard to write.
L<Marpa::UrHTML> uses L<HTML::PullParser> to do most
of the low-level parsing.
L<HTML::PullParser> needs to be wrapped with
other logic
to implement the wish-granting.

=head1 HOW THE WISH-GRANTING WORKS

First, let's consider the case of HTML valid
according to one of the standard.
What the Wish-Granting Tokenizer needs to do for
each token is
the following.
...
=item 1.
Get a list of the token types that the Wishful Thinking Parser
wants to see at this point.

=item 2.
If the physical token is one of them, hand it over to the
Wishful Thinking Parser.

=item 3.
If the physical token is not one of the token types, then
look in the list of tokens that the
Wishful Thinking Parser wants for an end tag, or a
start tag which is allowed to be optional according to the
standards.

=item 4.
Step 3 will always find one and only one token.
Invent a virtual token of that type and hand it over
to the Wishful Thinking Parser.

=back

For HTML valid according to the standards, these simple rules are
all that is needed.
Click the heels on the ruby slippers three times and you're there.

=head1 EVERYTHING LOOKS LIKE HTML

But L<Marpa::UrHTML> does not require its input document be
valid according to the HTML standards.
Far from it.
For L<Marpa::UrHTML> every document is a valid HTML documents.
...
Parsing the things that pass for HTML,
and making a reasonable guess as to the intent,
makes things
more complicated.
But the logic involved still is straightforward
and a natural outgrowth of the problem.

The rest of this document describes those additional
details needed for extremely loose HTML parsing.
...
but I decided that it was important for L<Marpa::UrHTML> to pass
the same tests as L<HTML::Tree>, so these three start tags
are also virtual terminals.

=head2 Interaction of the wishful-thinking-BNF and the Lexer.

Every once in a while in programming or mathematics you come across
an moment of real beauty.
This occurred for me when I combined the Wishful Thinking HTML BNF
with the predictive lexer.

The interaction is based on this simple rule:

     If you can't give the Wishful Thinking Grammar the token
     you have, give it the one it wants.

Since Marpa does predictive lexing, the UrHTML lexer can know which
virtual tokens will work.
B<It turns out that at almost all points there is almost
always just one virtual token that
the Wishful Thinking Grammar will accept.>
So it goes like this.

=over 4

=item The Wishful Thinking Grammar tells the Lexer what tokens it wants.

=item If the the actual physical token from HTML::TokeParser is the one
the Wishful Thinking Grammar wanted, the Lexer hands it over.

=item If the real, physical token is not acceptable to the Wishful Thinking Parser,
the Lexer looks in the list of tokens that the Wishful Thinking Parser will accept
for virtual tokens.  Usually there will be one and only one.

=item The Lexer gives the Wishful Thinking Grammar the token it wants.
...
=item *
It seemed desirable to allow for UrHTML someday to be configurable.

=item *
L<Marpa::UrHTML> is intended to encourage others to "borrow" its logic,
...
=head2 Prioritizing of Virtual Terminals
...
L<Marpa::UrHTML> deals with virtual C<< <table> >> tags by only applying them
...
=head2 Virtual tags: The Special Cases
...
That is, if in a block flow outside a table, L<Marpa::UrHTML>
...
but L<Marpa::UrHTML> keeping with its policy of liberalism, tries to produce valid
