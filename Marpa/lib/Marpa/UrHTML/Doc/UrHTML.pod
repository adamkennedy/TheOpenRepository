...
and at the top concatenating all values.
...
By default text is passed through unchanged, so the user
need only specify semantic actions for those components she
wants changed.
Here's how to change the title of a document:
...
=head2 Actual Structure vs. Element Tags
...
The traditional method requires semantic actions
to know precisely
what children every component will have.
This is processing model is not a good fit to HTML.
L<Marpa::UrHTML> gives the writer of semantics actions
"views" of each component which are natural
even when the number and types of whose
children is unknown or vague.
...
=head2 Views versus the Traditional Tree Semantics
...
views are more efficient than the traditional tree semantics.
In efficiency terms, the traditional model corresponds to a
situation where all components have handlers which return values,
and this is the view methods worst case, or close to it.

L<Marpa::UrHTML> optimizes for unvalued components.
Unvalued components are represented as token spans.
Adjacent descendant spans are merged as processing proceeds up the
hierarchy.
This means the size and time required do not increase as 
processing rises up the component hierarchy.

The original and token views
are calculated on a just-time-time basis
when they are requested through the action API.
The token view is produced from the token span.

Similarly, each token tracks the text it represents as a
character span.
The original text is calculated on a just-in-time basis
using the first character location of the first token and
the last character location of the last token.

When a handler does not need to return a value,
the most efficient thing for it to do is return C<undef>.
This allows that component and all its descendants to revert
to the efficient unvalued representation.

=head1 THE ACTION API
...
store the data.
It is often useful to have the action for the C<:TOP> pseudoclass
return
C<$Marpa::UrHTML::INSTANCE> as
the value of the parse.

=head2 Undefined Actions versus Actions Which Return C<undef>

Where no action is defined for a component, that component leaves the status quo intact.
That is, all values which were visible remain visible.
No new values become visible.

When an action is defined for a component, but that action returns undef,
all descendant values which were visible B<become invisible>.
In other words, all descendant values are destroyed.
Undefined actions do resemble actions which return C<undef> in one sense:
No new values become visible.

=head2 Root Element versus :TOP Pseudoclass

It is important not to confuse the C<:TOP>
component with the HTML root element.
...
The action
for C<:TOP> psuedoclass is always the last action to be called.
...
returns becomes the value of the parse.
It will be the value the C<Marpa::UrHTML::urhtml> method returns.

=back

None of these things are true of the root element.
...
Tags or not tags, every HTML document has a 
root element.
...
component.
They are never descendants of the root element.
