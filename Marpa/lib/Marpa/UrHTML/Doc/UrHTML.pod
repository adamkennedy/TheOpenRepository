...
It allows handlers to be specified for elements and other other major components
of an HMTL document, as well as its tokens.

     Perhaps the rest, or much of the rest, of this
     should go later.
...
Its arguments are zero or more references to hashes of named arguments.
...
Handler specifiers follow a format similar to CSS specifiers.
...
L<Marpa::UrHTML> pseudoclass specifiers are similar to CSS pseudo-class
specifiers -- they begin with a colon.
L<Marpa::UrHTML>'s own pseudoclasses must always be specified in upper-case.
Lowercase is reserved for CSS pseudoclasses,
none of which are currently supported.
