...
Marpa::UrHTML::Parsing_HTML - A Strategy for "Natural" HTML Parsing
...
on which L<Marpa::UrHTML> is based,
...
L<Marpa::UrHTML> uses L<HTML::PullParser> to handle most
...
L<Marpa::UrHTML> does not require its input document be
valid according to the HTML standards.
Far from it.
For L<Marpa::UrHTML> every document is a valid HTML documents.
L<Marpa::UrHTML> accepts that a document might be invalid or
broken, but
L<Marpa::UrHTML>
...
L<Marpa::UrHTML> is completely liberal
in what it accepts.
Since in broken HTML any end tag could be
missing, 
L<Marpa::UrHTML> is be prepared as a last resort
to supply a virtual replacement for any end tag.
This means that for
L<Marpa::UrHTML> all end tags are in effect optional.
