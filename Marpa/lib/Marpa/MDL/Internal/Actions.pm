...
sub non_structural_production_sentence { q{ priority => } . $_[1] }
...
sub default_lex_prefix_subject {
             return q{$new_default_lex_prefix = }
             . $_[0]
             . qq{;\n}
}
...
sub default_null_value_subject {
             return q{$new_default_null_value = }
             . $_[0]
             . qq{;\n}
}
...
sub symbol_phrase {
 Marpa::MDL::canonical_symbol_name(join(q{-}, @_)) }
...
sub optional_rhs_element {
     ...
     if (not defined $implicit_rules{$optional_symbol_phrase}) {
         ...
         push
             ...
             . q{ action => '} . __PACKAGE__ . q{::first_arg'}
         ;
         push
             @implicit_rules,
             q{ lhs => '} . $optional_symbol_phrase . q{', }
