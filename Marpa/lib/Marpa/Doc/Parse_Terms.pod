...
excellent chapter on parsing in the Perl context|Marpa::Bibliography/"Dominus 2005">.
It's available on-line.
L<Wikipedia|Marpa::Bibliography/"Wikipedia"> is also an excellent place to start.
...
A grammar which contains a cycle is B<infinitely ambigious>.
This is a grammar with a cycle can repeat the cycle in a derivation
an arbitrary number of times.
Therefore, for some input, there is no limit to the number
of parses a grammar with a cycle can produce.

A B<proper context-free grammar> in one which is
cycle-free and has no useless rules or empty productions.
Marpa does not require the context-free grammars it parses to
be proper.  Marpa can handles grammars with cycles, useless rules
or both.
