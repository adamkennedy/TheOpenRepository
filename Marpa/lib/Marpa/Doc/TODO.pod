...
=item * Add an enable_buggy_features option (default off).
Only allow cycles if it is on.
...
=item * Create a "stress test" for zero-width and-nodes and or-nodes:
A grammar with two rules in a cycle.  Each rule is all nulling
(nullable?) symbols except for the last symbol, which must be non-nulling.
