...
=item * LinkExtor replacement

=item * Delete comments

=item * Print out the title element

=item * Print out all tables, not duplicating any inside another table.

=item * Formatter which indents, adds missing tags,
and adds a comment on cruft.
