...
Marpa::Engine - How to use the Marpa Parse Engine

=head1 SYNOPSIS

=head1 DESCRIPTION

This is the top-level document describing how to use the Marpa parse engine itself.
Using a parser involves these three things:

=over

=item * Write a grammar.

=item * Recognize input.

=item * Evaluate the parse and produce a value.

=back

In Marpa these are essentially three distinct steps.
L<Marpa::Grammar> is the package which creates and precomputes
grammars.
L<Marpa::Recognizer> is the package which takes a precomputed grammar and
recognizes input using it.
In many cases
you can produce the parse's value quickly directly with
the L<Marpa::Recognizer>.

L<Marpa::Recognizer>'s fast built-in evaluator is far more flexible
than that in most parser generators.
It will handle any grammar which is not ambiguous --
where there is only one way to parse the input.
It will also handle ambigious grammars as long as
the grammar is not B<infinitely> ambigious, and
a single parse of the input, chosen at random, is all you want.

You will need a more powerful evaluator if

=over

=item * Your grammar is infinitely ambiguous.

=item * In an ambiguous parse, you want more than one of the possible parses.

=item * In an ambiguous parse, you want to be able to choose which one you get.

=back

If you have any of the above three requirements, you will need
