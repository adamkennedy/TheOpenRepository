...
L<a separate document on debugging|Marpa::Doc::Debugging>.
...
C<infinite_action> cannot be changed after the grammar is precomputed.
...
Marpa always ends cycles after the first iteration,
that is, before any symbol cycles back to itself.
Put another way, while Marpa allows grammars with cycles,
it will not produce a parse which contains any.
...
Who knows, there may be uses for cycles.
