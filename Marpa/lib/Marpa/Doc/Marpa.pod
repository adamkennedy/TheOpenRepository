...
These scripts are demos of L<Marpa::UrHTML>,
...
(L<Marpa::UrHTML> uses L<HTML::Parser>
as its low-level parser.)

=head1 GUIDE TO THE MARPA DOCUMENTATION

An incomplete user document for
L<Marpa::UrHTML> is now available.

Marpa's documentation is being brought up to date.
What is ready at the moment is only of interest
...
in the (mostly not yet released) documents.
