...
Marpa::Doc::Tutorial - A Marpa Tutorial

=head1 OVERVIEW

This Tutorial expands on an example in
L<the Marpa SYNOPSIS|Marpa/"SYNOPSIS">.
...
grammar.
    ...
    my $g = new Marpa( mdl_source => \$source);
...
parse object for each test line.
	...
	my $parse = new Marpa::Recognizer(grammar => $g);
...
text returns the number of the character where parsing was "exhausted".
This is the point where Marpa realizes no more successful parses will be found.
...
C<text()> returns -1 on success.  That's non-standard, but parsing can be exhausted at the very
