...
while the value of the C<rules> named argument is an array,
because symbols are uniquely idenitifed by name
which makes a convenient hash key.
For rules, there is
no convenient and obvious candidate for a hash key.
...
In the key/value pairs of this hash,
the hash key is the rule property names
and the hash value is the value of
that property.
...
The C<null_action> symbol property expresses semantics.
Its standard use is as the name of an action which
resolves to a Perl closure.
The C<null_action> Perl closure is
used at evaluation setup time to calculate the
null value of the symbol.
Details are given in
L<the document on semantics|Marpa::Semantics>.
