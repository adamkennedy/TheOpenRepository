...
The package L<Marpa::Grammar> deals with Marpa's grammar objects.
Grammars are created with the C<new> method.
Rules and options may be specified when the grammar is created, or later using
the C<set> method.

When the C<precompute> is called,
...
precomputation, but
no new rules may added and most other options are invalid.

=head2 Symbol Names

Any valid Perl string not ending in a right square bracket is an acceptable
symbol name.
Symbol names which end in right square brackets are reserved for Marpa internal use.

=head2 Terminals

B<Terminals> are symbols which are acceptable in the input --
as <token symbols>.
Marpa is unusual in allowing terminals to appear on the left
hand side of rules.
...
If the C<terminals> named argument explicitly marks
any symbol as a terminal, then
B<all its terminals> must be explicitly listed
in one of the C<terminals> named arguments.
...
returns a new grammar object or throws an exception.

The arguments to
C<Parse::Marpa::Grammar::new> are
references to a hashes of named arguments.
...
=head1 TRACE ACCESSOR METHODS

=head2 inaccessible_symbols
...
=head2 show_accessible_symbols

    say 'Accessible symbols: ',
        $grammar->show_accessible_symbols;

Returns a one-line string with the names
...
=head2 Named Arguments for Tracing
...
cannot be set after the grammar is precomputed.
...
in the named argument hash in which it is set.
...
=head2 The action Rule Property
...
the value of Marpa's C<default_action> option.
If the C<default_action> option is not
set, a rule's value will always be a Perl C<undef>.

=head2 keep Rule Property
...
=head2 The ranking_action Rule Property
