...
=item * Add strict actions option.  Action are found only
in a specified package, or as fully qualified names.  They
are not looked for in the Marpa caller's package.
