...
=item * Add code to abend if SELF_ARG is on.

=item * Change bin/mdl, ::mdl(), all *.t, boostrap to unset SELF_ARG

=item * Default to SELF_ARG on.  Test & fix what breaks.

=item * Implement actions objects, & use of SELF_ARG.

=item * One by one, convert, removing unset of SELF_ARG as I go.
Do NOT convert bootstrap, ../bin/mdl, or ::mdl().  They will be
removed after conversion to the new bootstraping, and the
elimination of MDL.
...
=item * After MDL is factored out, create a matrix of tests of 
...
=item * Prediction of lexables seems to have problems.  In particular,
ah2.t never predicts a lexable, though there should be one at every
earleme.

=item * Factor out the MDL Lexer.
