...
It returns a C<ORDB::CPANTS::Uses> object, or throws an exception if the
...
Returns a list of B<ORDB::CPANTS::Uses> objects when called in list context, or a
reference to an C<ARRAY> of B<ORDB::CPANTS::Uses> objects when called in scalar
context.
...
The uses table was originally created with the
