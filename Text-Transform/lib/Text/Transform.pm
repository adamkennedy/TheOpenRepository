...
Text::Transform - Abstract base class for document transformation classes

=head1 DESCRIPTION

C<Text::Transform> provides an API for the creation of classes and objects
that modify or transform text documents.
...
sub register_apply_handler {
	my $class   = shift;
	my $handler = _CLASS(shift) or Carp::croak("Invalid Text::Transform->register_apply_handler param");
	my $get     = _CODE(shift)  or Carp::croak("Invalid Text::Transform->register_apply_handler param");
	my $set     = _CODE(shift)  or Carp::croak("Invalid Text::Transform->register_apply_handler param");
	if ( $HANDLER{$handler} ) {
		Carp::croak("Text::Transform->apply handler '$handler' already exists");
	}
...
=head2 new
  ...
  my $transform = Text::Transform->new(
      param1 => 'value1',
      param2 => 'value2',
  );

The C<new> constructor creates a new object for your C<Text::Transform>
...
Returns a new C<Text::Transform>-compatible object, or returns
C<undef> on error.
...
The C<text> method should be implemented by each subclass, and
takes a single argument of a SCALAR reference, modifying it
...
that wasn't a SCALAR reference, but may include additional errors
depending on the subclass.

=cut

sub text {
	die "$class does not implement the required ->text method";
}
...
argument must be a L<Text::Document>, or something which can be turned
...
sub apply {
	...
	# Get, change, set
	my $Document = _INSTANCE($handler->[0]->($it), 'Text::Document')
		or Carp::croak("->apply handler for $class failed to get a Text::Document");
...
sub file {
	...
	# Process the file
	my $Document = Text::Document->new( "$input" ) or return undef;
	$self->document( $Document )                  or return undef;
...
See the L<support section|Text/SUPPORT> in the main module.
