...
sub SearchExe {
    ...
    if ($me->{sys} eq 'w') {
    	my ($clef,$type,$value);
    	# on essaie de piquer le chemin d'install par la clef d'ex‚cution.
...
sub initialize {
    ...
    if ($^O eq 'MSWin32') {
		...
		if ($@ ne '') {
			die "Impossible de charger le module Win32::Registry";
		}
    	$me->{sys} ='w';
    } else {
    	$me->{sys} ='';
...
jean-marc boulade E<lt>F<jmbperl@hotmail.com>E<gt>

=head1 COPYRIGHT

Copyright (c) 2002 jean-marc boulade. All rights reserved.
