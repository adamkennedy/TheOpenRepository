...
sub special_targets {
    ...
    $make_frag .= <<'MAKE_FRAG' if $self->make =~ /\bdmake\b/;
.USESHELL :
MAKE_FRAG
...
sub pasthru {
    my($self) = shift;
    return "PASTHRU = " . ($self->make =~ /\bnmake\b/ ? "-nologo" : "");
}
...
sub cd {
    ...
    return $self->SUPER::cd($dir, @cmds) unless $self->make =~ /\bname\b/;
