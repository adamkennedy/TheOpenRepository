...
#  ifdef IAMSUID
/* Drop scriptname */
#    define validate_suid(validarg, scriptname, fdscript, suidscript, linestr_sv) S_validate_suid(aTHX_ validarg, fdscript, suidscript, linestr_sv)
#  else
/* Drop suidscript */
#    define validate_suid(validarg, scriptname, fdscript, suidscript, linestr_sv) S_validate_suid(aTHX_ validarg, scriptname, fdscript, linestr_sv)
#  endif
#else
#  ifdef SETUID_SCRIPTS_ARE_SECURE_NOW
/* Drop everything. Heck, don't even try to call it */
#    define validate_suid(validarg, scriptname, fdscript, suidscript, linestr_sv) NOOP
#  else
/* Drop almost everything (well, almost on 5.10.x. Everything on 5.8.x) */
#    define validate_suid(validarg, scriptname, fdscript, suidscript, linestr_sv) S_validate_suid(aTHX)
#  endif
#endif
...
perl_construct(pTHXx)
{
   ...
   /* Init the real globals (and main thread)? */
    if (!PL_linestr) {
	/* Dead code elimination will tidy this away for nearly everyone.  */
...
Perl_moreswitches(pTHX_ char *s)
{
	...
	if (*s) {
	    ...
	    Safefree(e);
	    /* This cast is a side effect of the Bowdlerisation of 5.8.x  */
	    s = (char *)p;
	    if (*s == '-')
		s++;
	}
