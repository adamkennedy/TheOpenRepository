...
sub add_directory {
    ...
    # Check required parameters.
    if ( (
               ( not defined $params_ref->{special} )
...
sub as_string {
    ...
    # Short circuits...
    if ( ( $string eq q{} ) and ( $self->special == 0 ) and ( $tree == 0 ) )
    {
        return q{};
    }
    ...
    if ( ( defined $string ) && ( $string ne q{} ) ) {
        if ( $self->special == 2 ) {
            ...
            $answer =
              "<Directory Id='D_$self->{id}' Name='$self->{name}'>\n";
            $answer .= $self->indent( 2, $string );
            $answer .= "\n</Directory>\n";
        }
    } else {
