...
sub new {
	...
	unless ( defined $params{fragment_dir} ) {
		$params{fragment_dir} = catdir($params{temp_dir}, 'wix_fragments');
...
sub install_dmake {
	...
	$self->{bin_make} = catfile($self->image_dir, 'c', 'bin', 'dmake.exe');
...
sub install_binutils {
	...
	$self->{bin_dlltool} = catfile($self->image_dir, 'c', 'bin', 'dlltool.exe');
...
sub install_pexports {
	...
	$self->{bin_pexports} = catfile($self->image_dir, 'c', 'bin', 'pexports.exe');
...
sub install_binary {
	...
	if ( ref $binary->install_to eq 'HASH' ) {
		@files = $self->_extract_filemap( $tgz, $binary->install_to, $self->image_dir );
...
sub install_library {
	...
	if ( _HASH($library->build_a) ) {
		...
		push @files, $self->_dll_to_a(
			...
			(
				source => catfile($unpack_to, $library->build_a->{source}),
			) : (),
...
sub _search_packlist {
    ...
    my $site = catfile(catdir($self->image_dir, qw{perl site lib auto}, split('::', $module)), '.packlist');
    my $fl;
...
sub install_launcher {
	...
	my $to = catfile( $self->image_dir, 'perl', 'bin', $launcher->bin . '.bat' );
	unless ( -f $to ) {
		...
		for my $file ( $tar->get_files ) {
			...
			for my $tgt ( keys %$filemap ) {
				...
				if ( $file_only ) {
					...
					($t = $canon_f)   =~ s{\A([^/]+[/])?\Q$canon_tgt\E\z}{$filemap->{$tgt}}i;
