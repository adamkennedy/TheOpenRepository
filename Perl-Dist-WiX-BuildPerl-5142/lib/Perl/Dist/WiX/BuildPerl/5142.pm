...
Perl::Dist::WiX::BuildPerl::5142 - Files and code for building Perl

=head1 DESCRIPTION

This module provides the routines and files that Perl::Dist::WiX uses in 
order to build Perl 5.14.2  
...
around '_install_perl_plugin' => sub {
  ...
  my $perl = Perl::Dist::WiX::Asset::Perl->new(
    parent => $self,
    url    => 'http://search.cpan.org/CPAN/authors/id/F/FL/FLORA/perl-5.14.2.tar.gz', #XXX-FIX-ON-VERSION-BUMP
    patch  => [ qw{
      lib/CPAN/Config.pm
      win32/config.gc
...
around '_find_perl_file' => sub {
  ...
  $location = eval {
    File::ShareDir::module_file( 'Perl::Dist::WiX::BuildPerl::5142', "default/$file" ); #XXX-FIX-ON-VERSION-BUMP
  };
...
has 'perl_version_literal' => (
  is       => 'ro',
  init_arg => undef,
  default  => '5.014002', #XXX-FIX-ON-VERSION-BUMP
);
...
has 'perl_version_human' => (
  ...
  init_arg => undef,
  default  => '5.14.2', #XXX-FIX-ON-VERSION-BUMP
);

has '_perl_version_arrayref' => (
  is       => 'ro',
  init_arg => undef,
  default  => sub { [ 5, 14, 2 ] }, #XXX-FIX-ON-VERSION-BUMP
);

has '_perl_bincompat_version_arrayref' => (
  is       => 'ro',
  init_arg => undef,
  default  => sub { [ 5, 13, 31 ] }, #XXX-FIX-ON-VERSION-BUMP
