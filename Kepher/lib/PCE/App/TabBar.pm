package PCE::App::TabBar;    # notebook file selector
$VERSION = '0.10';
...
sub _get_tabs { $PCE::app{window}{(APPROOT)}{tabs} }
sub _set_tabs { $PCE::app{window}{(APPROOT)}{tabs} = shift }
sub _get_sizer{ $PCE::app{window}{(APPROOT)}{sizer} }
sub _set_sizer{ $PCE::app{window}{(APPROOT)}{sizer} = shift }
sub _get_config{$PCE::config{app}{tabbar} }
#sub new{ return PCE::App::Window::_get()->{'notebook'} = Wx::Notebook->new($frame, -1, [0,0], [1,1],)}
...
sub create {
	...
	my $cmd_new_data = PCE::App::CommandList::get_cmd_properties('file-new');
	if (ref $cmd_new_data->{'icon'} eq 'Wx::Bitmap'){
		...
		EVT_ENTER_WINDOW( $new_btn, sub {
			PCE::App::StatusBar::info_msg( $cmd_new_data->{'help'} )
		});
		EVT_LEAVE_WINDOW( $new_btn, \&PCE::App::StatusBar::refresh_info_msg );
	}

	my $cmd_close_data = PCE::App::CommandList::get_cmd_properties('file-close');
	if (ref $cmd_close_data->{'icon'} eq 'Wx::Bitmap'){
		...
		EVT_ENTER_WINDOW($close_btn, sub {
			PCE::App::StatusBar::info_msg( $cmd_close_data->{'help'} )
		});
		EVT_LEAVE_WINDOW( $close_btn, \&PCE::App::StatusBar::refresh_info_msg );
	}
	...
	EVT_MIDDLE_UP( $tabbar->{tabs}, PCE::App::CommandList::get_cmd_property
...
sub refresh_label {
	...
	my $doc_nr = shift;
	$doc_nr ||= 0;
	return unless defined $PCE::internal{'document'}{'open'}[$doc_nr];

	my $doc_internals = \%{ $PCE::internal{'document'}{'open'}[$doc_nr] };
	my $label         = $doc_internals->{'name'};
	$label = "<$PCE::localisation{app}{tabbar}{untitled}>" unless $label;
...
sub refresh_current_label { refresh_label( PCE::Document::_get_current_nr() ) }

sub refresh_all_label {
	if ($PCE::internal{'document'}{'loaded'}){
		refresh_label($_) for 0 .. PCE::Document::_get_last_nr();
		set_current_page( PCE::Document::_get_current_nr() );
	}
}
...
sub show {
	my $main_sizer = PCE::App::Window::_get()->GetSizer;
...
sub left_on_tabs {
	my ($tabs, $event) = @_;
	$PCE::internal{'document'}{'b4tabchange'} = $tabs->GetSelection;
	$event->Skip;
}
sub left_off_tabs {
	my ($tabs, $event) = @_;
	PCE::Document::Change::switch_back()
		if $PCE::internal{'document'}{'b4tabchange'} == $tabs->GetSelection;
	$event->Skip;
}
sub change_tab {
	my ( $frame, $event ) = @_;
	PCE::Document::Change::to_number($event->GetSelection);
	$event->Skip;
}
