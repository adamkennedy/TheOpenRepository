package Kephra::App::CommandList;
$VERSION = '0.06';
...
sub _get_data  { $Kephra::app{'command'} }
...
sub assemble_data{
	...
	_copy_conf_values($Kephra::localisation{(CFGROOT)}{label},'label');
	_copy_conf_values($Kephra::localisation{(CFGROOT)}{help}, 'help');
	_create_keymap()
}
...
sub _copy_conf_values {
	...
	local $list = \%{ $Kephra::app{(APPROOT)} }; # commandlist data
...
sub del_temp_data{
	delete $Kephra::localisation{(CFGROOT)}
		if exists $Kephra::localisation{(CFGROOT)};
}
