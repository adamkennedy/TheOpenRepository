...
sub _get{ $Kephra::app{(APPROOT)}{ $_[0] } }
sub _set{ $Kephra::app{(APPROOT)}{ $_[0] } = $_[1] if ref $_[1] eq 'Wx::ToolBar'}
...
sub eval_data {
	...
	my $bar_item_id = exists $Kephra::app{(APPROOT)}{item_id}
		? $Kephra::app{(APPROOT)}{item_id}
		: $Kephra::app{GUI}{masterID}++ * 100;
	$Kephra::app{(APPROOT)}{item_id} = $bar_item_id;
	my $item_kind;
