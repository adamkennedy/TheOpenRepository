...
sub set { $Kephra::app{(APPROOT)}{$_[0]}{ref} = $_[1] if ref $_[1] eq 'Wx::Menu' }
sub get { $Kephra::app{(APPROOT)}{$_[0]}{ref}
	if ref $Kephra::app{(APPROOT)}{$_[0]}{ref} eq 'Wx::Menu'
}
...
sub create_dynamic {
	...
	if ($menu_name eq '&file_history'){
		...
		set_update($menu_id, sub {
			...
			#$menu_data[3]{type} = '';
			for my $nr (0 .. Kephra::Document::_get_last_nr()){
		...
		add_onopen_check( $menu_id, 'select', sub {
			...
			$menu->Check (
				...
				Kephra::Document::_get_current_nr(), 1
			);
		});
	}
...
sub eval_data {
	...
	my ($item_id, $kind);
	$item_id = exists $Kephra::app{(APPROOT)}{$menu_id}{item_id}
