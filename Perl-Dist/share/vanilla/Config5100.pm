...
# Locate $Config{perlpath} directly
our $perlpath = $^X;
unless ( -e $perlpath ) {
    die('Failed to auto-discover $Config{perlpath}');
}

# Derive $Config{bin} from $Config{perlpath}
our $bin = $perlpath;
$bin =~ s/(?:\\\\?|\/)perl\.exe$//;
unless ( $bin ne $perlpath and -d $bin ) {
    die('Failed to auto-discover $Config{bin}');
}

# Derive $Config{prefix} from $Config{bin}
our $prefix = $bin;
$prefix =~ s/(?:\\\\?|\/)bin$//;
unless ( $prefix ne $bin and -d $prefix ) {
