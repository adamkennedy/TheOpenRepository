...
sub which {
	...
	unless ( $location ) {
		croak( "Can't find the required program '$program'. Please install it" );
	}
	unless ( -r $location and -x $location ) {
		croak( "The required program '$program' is installed, but I do not have permission to read or execute it" );
	}
	return $location;
}
...
sub shell {
	...
	croak( $_[0] || "Failed to run '$command'" );
}
...
sub copy {
	...
	croak( "Failed to copy '$from' to '$to'" );
}
...
sub move {
	...
	print "- move '$from' => '$to'\n" if $VERBOSE;
	File::Flat->copy( $from => $to ) and return 1;
	croak( "Failed to move '$from' to '$to'" );
}
...
sub remove {
	...
	croak( "Failed to remove '$path'" ) if -e $path;
	return 1;
}
